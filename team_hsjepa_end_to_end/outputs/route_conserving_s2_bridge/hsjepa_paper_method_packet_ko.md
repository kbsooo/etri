...
- Big-bet queue: `big_bet_queue_ready` (`22` bets)
...
- Frontier active-silence solver: `candidate_ready`
- Frontier active-silence recommended variant: `positive_path_overshoot_sensor`
- Frontier bad first-mode variance: `0.9629`
- Frontier active-silence cells: `3355`
...
- Frontier active-silence: `candidate_ready`, recommended `positive_path_overshoot_sensor`, file `submission_hsjepa_frontier_silence_positive_path_overshoot_sensor_1e013277_uploadsafe.csv`, cells `3355`, changed cells `38`, first bad-mode variance `0.9629`, frontier cosine `0.5793`, bad-tangent cosine `-0.3504`, silence `0.1962`
...
- `Frontier-Trajectory Active-Silence Solver`: The public frontier is a noisy descent trajectory; useful HS-JEPA actions continue the positive frontier tangent while actively silencing post-frontier toxic branches. Expected LB delta if true `-0.016`. Kill: Positive-path, anti-silence, continuation, and boundary variants all fail public LB, meaning the H012->H057 trajectory was a local leaderboard path rather than an action-grade hidden-state descent direction.
...
- `/Users/kbsoo/Downloads/cl2/sleep_competition_adapter/outputs/frontier_trajectory_silence_solver/frontier_trajectory_silence_readout.json`
