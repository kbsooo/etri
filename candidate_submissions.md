...
## Target-Listener Route Lift Candidate Update
...
Target-listener posterior is useful, but not yet row-assignment-grade.
...
So this candidate is not a safe improvement candidate.  It is a public sensor
for one precise claim:

```text
Can S2 listener posterior discover route-safe extra S2 cells beyond the
stagebridge/S2hub teacher?
```

If it wins, HS-JEPA gets a new module:
...
## Sleep-Pressure World Model Probe
...
- doc: `paper_hsjepa_core/SLEEP_PRESSURE_WORLD_MODEL_CORE_KO.md`
- status: upload-safe downstream probe candidate
- public LB: not submitted
- semantic purpose: night disturbance, physiological load, social/cognitive arousal, rest-environment stability를 label-free sleep-pressure hidden target으로 만들고 low-trust frozen probe로 Q/S signal을 읽는다.
...
```text
보이는 daily human-life context만으로
보이지 않는 sleep-pressure / arousal / recovery-load representation을 예측하면
Q/S label manifold가 조금 더 선형화되는가?
```

### Local Evidence

- best pretext: `masked_context_to_sleep_pressure_family / social_cognitive_arousal`
- component-corr lift vs null: `+0.563048`
- R2 lift vs null: `+0.499132`
- subject-heldout prior logloss: `0.677858`
...
- raw PCA subject-id accuracy: `0.957778`
- validation: valid, rows `250`, probability range `[0.451605, 0.706724]`

### Submission Priority

낮음에서 중간. 최고 LB용 한탕 후보라기보다 HS-JEPA core sensor다.
pretext signal은 매우 강하지만 label probe 효과는 작아서, public에서 큰 개선을 기대하기보다는
sleep-pressure representation이 public subset에서도 살아나는지 확인하는 의미가 크다.

좋아지면:

- sleep-pressure surrogate가 public에서도 Q/S label manifold와 연결된다는 뜻이다.
- 다음 제출 후보는 sleep-pressure를 단독 probe로 쓰는 것이 아니라, routine-break/listener responsibility와 결합해야 한다.

나빠지면:

- local 해석과 일치한다. sleep-pressure는 core representation으로는 강하지만,
  label/action decoder 없이 바로 제출값으로 번역하면 약하다는 뜻이다.

## Cohort-Relative World Model Probe

- file: `submission_hsjepa_cohort_relative_world_model_probe_24fd334b_uploadsafe.csv`
- code: `hsjepa_core/run_cohort_relative_world_model_core.py`
- doc: `paper_hsjepa_core/COHORT_RELATIVE_WORLD_MODEL_CORE_KO.md`
- status: upload-safe downstream probe candidate
- public LB: not submitted
- semantic purpose: routine-break와 sleep-pressure 기반 subject fingerprint로 singleton 없는 peer cohort를 만들고, predicted personal-vs-peer hidden state만 low-trust frozen probe로 Q/S signal에 연결한다.
...
```text
보이는 daily human-life context만으로
개인 기준과 peer cohort 기준에서 오늘이 어떤 상태인지 예측하면
subject-heldout Q/S label manifold가 더 선형화되는가?
```

중요한 점은 observed cohort state가 아니라 predicted cohort state를 쓴다는 것이다.
observed/full cohort geometry는 subject shortcut이 강하다.

### Local Evidence

- cohort count: `2`
- cohort sizes: `{0: 5, 1: 5}`
- best pretext: `visible_context_to_cohort_relative_state / all_visible_context`
- component-corr lift vs null: `+0.672489`
- R2 lift vs null: `+0.572030`
- subject-heldout prior logloss: `0.677858`
- cohort-relative predicted calibrated logloss: `0.676477`
- cohort-relative predicted delta vs prior: `-0.001381`
- cohort-relative full delta vs prior: `+0.001294`
- predicted subject-id accuracy: `0.353333` vs chance `0.126667`
- observed subject-id accuracy: `0.962222`
- validation: valid, rows `250`, probability range `[0.454990, 0.712210]`

### Submission Priority

중간. 최고 LB용 확정 후보라기보다 HS-JEPA core sensor로 정보량이 크다.
public에서 좋아지면 cohort-relative predicted state가 실제 test/public hidden structure와 연결된다는 뜻이다.

좋아지면:

- HS-JEPA에 `personal-vs-peer cohort coordinate prediction`이라는 강한 논문 모듈을 넣을 수 있다.
- 다음 후보는 이 predicted cohort state를 listener responsibility/action-health decoder와 결합해야 한다.
...
- primary file: `submission_hsjepa_human_state_drift_consistency_drift_consistency_overshoot_0fb93301_uploadsafe.csv`
- control file: `submission_hsjepa_human_state_drift_consistency_certified_replay_76bb1a88_uploadsafe.csv`
- code: `sleep_competition_adapter/human_state_drift_consistency_certifier.py`
...
- paper doc: `paper_hsjepa_core/HUMAN_STATE_DRIFT_CONSISTENCY_CERTIFIER_KO.md`
- team brief: `paper_hsjepa_core/HUMAN_STATE_DRIFT_CONSISTENCY_TEAM_BRIEF_KO.md`
- status: control submitted and strongly positive; overshoot remains unsubmitted challenger
- known public LB for control: `0.5619100863`

### Why This Candidate Exists

이 후보는 "public LB를 LP로 맞춘다"가 아니라, 그 결과를 HS-JEPA의 인간 이해 언어로 다시 세운다.

```text
같은 subject의 test rows는 서로 독립 cell이 아니라,
같은 recovery/degradation human-state drift field를 공유한다.
```

그래서 row마다 다른 sparse surgery를 하지 않고, subject 내부 모든 row에 같은 Q2/Q3 logit move를 준다. 이 구조는 assignment noise를 줄이고, train history와 aggregate listener가 동시에 가리키는 subject state direction만 action으로 번역한다.

### Candidate Anatomy

- base: `submission_hsjepa_frontier_silence_positive_path_overshoot_sensor_1e013277_uploadsafe.csv`
- targets changed: `Q2`, `Q3`
- active subjects: `id02`, `id03`, `id04`, `id06`, `id07`, `id08`, `id09`, `id10`
- neutral subjects: `id01`, `id05`
- changed rows: `202`
- changed cells: `404`
- upload safe: `True`
- control public LB: `0.5619100863`
- delta vs FrontierSilence `0.5677269444`: `-0.0058168581`
- delta vs external certified `0.5647490904`: `-0.0028390041`

Subject drift field in the primary candidate:

```text
id03/id04/id06/id09: Q2/Q3 upward human-state action
id02/id07/id08/id10: Q2/Q3 downward human-state action
id01/id05: hold
```

Step size is larger when train-time drift supports the listener direction and smaller when it conflicts.

### Submission Priority

`certified_replay`는 이미 public에서 강하게 성공했다. 이 파일은 현재 팀 공유 기준의 핵심 재현 파일이다.

`drift_consistency_overshoot`는 다음 질문을 위한 challenger다.

```text
direction만 맞은 것인가, 아니면 train drift consistency로 magnitude까지 더 잘 정할 수 있는가?
```

제출 우선순위는 상황 의존이다. 남은 슬롯이 거의 없고 best 보존이 목적이면 이미 검증된 `certified_replay`를 기준으로 삼는다. 정보량을 원하면 `drift_consistency_overshoot`를 제출한다.

좋아지면:

- subject-level human-state drift consistency가 certified equation보다 더 좋은 magnitude decoder라는 뜻이다.
- HS-JEPA paper에는 `Human-State Drift Consistency Certifier`를 core-to-adapter bridge로 넣을 수 있다.
