...
Boundary-consensus update: I tested whether actual hidden blocks can be corrected by adjacent known labels. Submission exposure is large enough to matter (`same0/same1` rows exist across all targets, e.g. S3 has 107 `same1` rows), but the signal does not survive nested selection. Apparent gains on full OOF are tiny and misleading (`minimax + zeros_only` appears `-0.000337`), while nested validation worsens every tested base/mode. Best case is `stage2 + zeros_only`, still `+0.000266`; minimax `zeros_only` is `+0.000430`. Conclusion: the block-rate oracle gap is not recoverable by simple endpoint labels; it requires block-level covariates or a stronger label generator.
...
10a. Hidden-block boundary consensus is also not enough.  
    A stricter endpoint audit over real train analogue blocks found the same failure at the frontier level. Endpoint labels are often available around sample blocks, but target-wise nested selection turns every endpoint mode negative: `same`, `same_or_single`, `ones_only`, and `zeros_only` all worsen stage2/public2d0/proj0/minimax. Do not add boundary postprocessing unless a new covariate first identifies which block endpoints are trustworthy.
...
Public mask-plausibility update:

```text
scripts                   public_mask_plausibility_reweight.py, public_mask_plausibility_blend_scan.py
new public feedback        ordinal/ambnext public 0.5783033652, worse than stage2 by +0.0003583895
mask evidence              global prefix/suffix masks require much larger probability movement than all/random/subject masks
profile grid               6 mask priors x 4 scenario profiles = 24 ranking views
best mean-rank files        u65_r04 and u65_r01, both mean_rank 17.54 after adding blends
u65_r04                    OOF 0.554759, plausibility mean_score 0.576299, six-posterior score 0.575013
u65_r01                    OOF 0.554318, plausibility mean_score 0.576335, six-posterior score 0.574977
old entropy-trusted best    minimax r05, six-posterior score 0.5749169278
best compromise blends      maskplausens_r04/r05, mean_score about 0.576276-0.576278, but no profile wins
decision                   split the high-upside queue by assumption. If trusting the original entropy posterior, submit minimax r05/r01. If guarding against public-mask/pseudo-posterior uncertainty, submit u65_r04/u65_r01. Use maskplausens_r04/r05 only as compromise probes.
```
...
premise                   solve public-score constraints with one latent probability per actual submission block-target
actual hidden blocks       36
best minimax analogue      0.554332 -> 0.570921, delta +0.016588
best public2d0 analogue    0.561702 -> 0.573676, delta +0.011974
constraint residual        0.0
decision                   reject. Block rates are real, but three public scores cannot identify 252 block-target latents; use row-cell entropy/minimax instead.
```

Measurement-process / wear-compliance feature family:
...
scan policy                      target-balanced guard; 3805 core coverage/gap/watch-phone columns
...
Q2                               no selected feature; best raw Q2 move is only about -0.0001
public posterior                 rejected: best mp scenario score 0.587092, no-Q2/no-Q3 soft 0.591289
decision                         use as structural evidence and feature source, not as primary public submission.
```

Direction audit:

```text
Q1  more core5h usage rows predicts lower Q1; bottom rank label_rate 0.743 vs top rank 0.300.
Q3  higher pre6h HR coverage predicts lower Q3; bottom bin label_rate 0.780 vs top bin 0.411.
S2  longer watch-light gaps predict lower S2 after base adjustment.
S3  larger usage sampling gaps predict lower S3 after base adjustment.
S4  higher active-sensor count above same-weekend baseline predicts higher S4; bottom bin label_rate 0.408 vs top bin 0.708.
S1  weak binary watch-gap irregularity signal; keep only as combo support.
```

Q2-only residual update:

```text
script                         q2_publicsafe_residual_blend_scan.py
materializer                   q2_stage2safe_candidate_materializer.py
base                           submitted stage2 public best, 0.5779449757
bad control                    ordinal Q2/Q3 public 0.5783033652
best aggressive candidate       submission_q2_stage2safe_r02_369582d6.csv
aggressive evidence             OOF mean -0.001565, Q2 -0.010956, geometry Q2 win-rate 1.0
best conservative candidate     submission_q2_stage2safe_r10_a7aa0680.csv
conservative evidence           OOF mean -0.001044, Q2 -0.007307, geometry Q2 win-rate 1.0, public-axis -0.000005
decision                        use Q2-only stage2-safe files as information probes; do not use anchor-based Q2 blends or full Q2/Q3 ordinal constraints.
```
