...
boundary consensus          best nested delta +0.000266; no candidate promoted
```
...
The boundary-consensus audit closes another tempting split-structure route. Actual sample blocks frequently have same-label endpoints, but nested validation rejects endpoint postprocessing on stage2, public2d0, proj0, and minimax. Apparent minimax full-OOF improvement (`-0.000337`) collapses to `+0.000430` nested. Do not submit boundary-derived files; none were generated.
...
The latest public result demotes the aggressive ordinal/ambnext backbone:

```text
submission_ordinal_q_constraints_ambnext_Q2Q3_nearest_w0.75.csv
public LB          0.5783033652
vs stage2          +0.0003583895 worse
```

I then reweighted the public-subset audit by how plausible each mask family is under the three observed public constraints. Global prefix/suffix masks are outliers because they need much larger probability movement than all-row, random, rowmod, and subject-contiguous masks.

```text
scripts                         public_mask_plausibility_reweight.py, public_mask_plausibility_blend_scan.py
profile grid                     6 mask priors x 4 posterior profiles
best mean-rank                   submission_public_universeens_u65_r04_dc6f3303.csv
best profile-win count           submission_public_universeens_u65_r01_365a84a6.csv
best old six-posterior score      submission_public_minimaxens_r05_a10_h506746.csv
best compromise blend             submission_public_maskplausens_r05_e8325bb7.csv
```

High-upside public-constraint burn order:

1. `submission_public_universeens_u65_r04_dc6f3303.csv`  
   Best mask-plausibility mean-rank. OOF `0.554759`, plausibility mean score `0.576299`, six-posterior score `0.575013`.

2. `submission_public_universeens_u65_r01_365a84a6.csv`  
   Wins 6/24 plausibility profiles. OOF `0.554318`, plausibility mean score `0.576335`, six-posterior score `0.574977`.

3. `submission_public_minimaxens_r05_a10_h506746.csv`  
   Best under the original six-posterior/equal-mask assumption. Score `0.5749169278`, OOF `0.554344`; essentially tied with `r01`.

4. `submission_public_minimaxens_r01_a6_h422045.csv`  
   Sparse and simpler old minimax default. Score `0.5749169564`, OOF `0.554332`.

5. `submission_public_maskplausens_r05_e8325bb7.csv`  
   Compromise `70% u65_r04 + 30% u50_r02`. OOF `0.555222`, plausibility mean score `0.576278`, six-posterior score `0.575111`.

6. `submission_public_maskplausens_r04_161f5469.csv`  
   Compromise `70% u65_r04 + 30% u50_r01`. OOF `0.555232`, plausibility mean score `0.576276`, six-posterior score `0.575116`.

Use this list only for high-upside public-constraint submissions. The low-risk projection ladder below remains the safer diagnostic path if submission budget is scarce.

## Public Block-Entropy Rejection

The actual submission rows form 36 hidden blocks, and validation-block oracle loss is very low (`0.517878`). I tested whether this structure can be combined with the three public-LB constraints by solving one latent probability per block-target instead of one probability per row-target cell.

```text
script                         public_block_entropy_projection.py
hidden submission blocks         36
latent variables                 up to 36 blocks x 7 targets
best minimax-prior analogue       0.554332 -> 0.570921, delta +0.016588
best public2d0-prior analogue     0.561702 -> 0.573676, delta +0.011974
best proj0-prior analogue         0.562144 -> 0.573923, delta +0.011780
constraint residual              0.0 for saved files
integrity                        250 rows, no duplicate keys, no null predictions
```

Decision update: reject block-target public entropy submissions. The block oracle proves block rates matter, but three public aggregate scores do not identify 36x7 block-target probabilities. The row-cell entropy/minimax family remains safer.
...
## Latest Negative Controls

Ordinal Q-count constraints and measurement-process features both became useful diagnostics but not public-safe submissions:

```text
submission_ordinal_q_constraints_ambnext_Q2Q3_nearest_w0.75.csv  public 0.5783033652, worse than stage2 by +0.0003583895
best measurement-process single                                  submission_mp_q3off650_q3_hr.csv, scenario score 0.587092
best measurement-process no-Q3 soft                               submission_mp_q3off650_noq2_noq3_soft.csv, scenario score 0.591289
best measurement-process mask-plausibility rank                   about 110+, far behind u65/minimax
```

Queue implication: do not promote `submission_mp_*`, `submission_rhythm_*`, or full ordinal/ambnext files above the u65/minimax/public-constraint frontier. They are valuable for feature/label-process understanding and low-priority information probes only.

## Q2-Only Follow-Up From Latest Public Result

The new Q2 scan treats the submitted stage2 file as the public anchor and changes only Q2. This avoids the broad ordinal/ambnext failure mode.

Primary information probes:

```text
submission_q2_stage2safe_r02_369582d6.csv
  Q2 donor: public_maskaware_t50_r05
  OOF mean delta: -0.001565
  Q2 delta: -0.010956
  geometry Q2 delta: -0.011624, win-rate 1.0
  two-axis public delta vs stage2: +0.000008

submission_q2_stage2safe_r10_a7aa0680.csv
  Q2 donor: orthcap_s003_capm005
  OOF mean delta: -0.001044
  Q2 delta: -0.007307
  geometry Q2 delta: -0.007854, win-rate 1.0
  two-axis public delta vs stage2: -0.000005
```

Priority interpretation:

```text
1. If the next submit is allowed to be an information probe, use r02: largest Q2-only upside with almost neutral public-axis exposure.
2. If minimizing another public miss is more important, use r10 or r05: smaller OOF gain but slightly favorable public-axis direction.
3. Do not use the anchor-based q2_publicsafe_blend files; they improve anchor Q2 offline but inherit anchor non-Q2 weakness and are dominated by stage2-safe variants.
```
