...
OUT_SCAN = OUT / "raw05_jepa_energy_constrained_frontier_scan.csv"
OUT_SCORED = OUT / "raw05_jepa_energy_constrained_frontier_scored.csv"
OUT_SHORTLIST = OUT / "raw05_jepa_energy_constrained_frontier_shortlist.csv"
OUT_INTEGRITY = OUT / "raw05_jepa_energy_constrained_frontier_integrity.csv"
OUT_REPORT = OUT / "raw05_jepa_energy_constrained_frontier_report.md"

BETAS = [0.03, 0.05, 0.075, 0.10, 0.125, 0.15, 0.18, 0.22, 0.26, 0.30, 0.35, 0.40, 0.48]
...
def candidate_pools() -> tuple[list[str], list[str]]:
    ...
    bases: list[str] = []
    bases.extend(local.sort_values("actual_anchor_score_final")["file"].astype(str).head(42).tolist())
    bases.extend(
        local[local["bucket"].eq("raw_boundary")]
    ...
    bases.extend(
        local[local["bucket"].eq("very_low_bad")]
        .sort_values("bad_residual_axis_ratio")["file"]
        .astype(str)
        .head(10)
        .tolist()
    )
    ...
    donors: list[str] = []
    donors.extend(ctx.sort_values("actual_anchor_score_final")["file"].astype(str).head(30).tolist())
    donors.extend(
        ctx.sort_values(["bad_residual_axis_ratio", "actual_anchor_score_final"])["file"]
        .astype(str)
        .head(18)
        .tolist()
    )
    donors.extend(
        ctx.sort_values(["energy_delta_vs_base", "actual_anchor_score_final"])["file"]
        .astype(str)
        .head(18)
        .tolist()
    )
    donors.extend(
        local[local["bucket"].isin(["raw_boundary", "strict_raw", "very_low_bad", "posterior"])]
...
def blend_profiles() -> list[tuple[str, np.ndarray]]:
    return [
        ...
        ("flat_noq3s4", profile_vector(Q1=1.0, Q2=1.0, S1=1.0, S2=1.0, S3=1.0)),
        ("q2s1heavy", profile_vector(Q1=0.80, Q2=1.15, S1=1.15, S2=0.85, S3=0.85)),
        ("q1light", profile_vector(Q1=0.45, Q2=1.0, S1=1.0, S2=1.0, S3=1.0)),
        ("s4tiny_only_context", profile_vector(Q1=0.8, Q2=0.8, S1=0.8, S2=0.8, S3=0.8, S4=0.1)),
        ("q3s4_tiny", profile_vector(Q3=0.20, S4=0.20)),
    ]
...
def generate_candidates(
    ...
    for base_file in bases:
        ...
        for donor_file in donors:
            ...
            step = donor_logit - base_logit
            for profile_name, profile_gate in profiles:
                gated_step = step * profile_gate
                if np.abs(gated_step).mean() <= 1e-10:
                    continue
                for beta in BETAS:
                    pred_logit = base_logit + beta * gated_step
                    pred = sigmoid(pred_logit)
                    final_energy = energy_rows(pred_logit - raw_logit, model)
                    label = f"{base_file}|donor={donor_file}|profile={profile_name}|b={beta:.3f}"
                    meta = {
                        "base_file": base_file,
...
def prefilter(scan: pd.DataFrame) -> pd.DataFrame:
    ...
    specs = [
        ...
        (
            ...
            & (frame["delta_vs_raw05_rawaxis"] <= 1.3e-7)
...
def score_candidates(selected_for_scoring: pd.DataFrame, preds: list[np.ndarray], sample: pd.DataFrame) -> pd.DataFrame:
    ...
    scored["selection_score"] = (
        ...
        + np.maximum(scored["bad_residual_axis_ratio"].abs() - 0.00155, 0.0) * 0.060
    ...
    scored["guardrail_score"] = (
        ...
        + np.maximum(scored["bad_residual_axis_ratio"].abs() - 0.00170, 0.0) * 0.050
        + np.maximum(scored["energy_delta_vs_base"], 0.0) * 0.00060
    )
    scored["lowbad_score"] = (
        scored["actual_anchor_score_final"]
        + np.maximum(scored["bad_residual_axis_ratio"].abs() - 0.00105, 0.0) * 0.080
...
def select_and_save(scored: pd.DataFrame, preds: list[np.ndarray], sample: pd.DataFrame) -> pd.DataFrame:
    specs = [
        (
            "frontier_actual",
            (scored["delta_vs_raw05_rawaxis"] <= 1.4e-7)
            & (scored["bad_residual_axis_ratio"].abs() <= 0.00165)
        ...
        (
            ...
            & (scored["delta_vs_raw05_rawaxis"] <= 1.3e-7)
    ...
    for _, row in selected.iterrows():
        ...
        file_name = f"submission_raw05_jepa_energyfront_{tag}.csv"
...
def write_report(scan: pd.DataFrame, scored: pd.DataFrame, selected: pd.DataFrame, integ: pd.DataFrame) -> None:
    ...
    best_by_profile = (
        ...
        .groupby(["blend_profile", "beta"], as_index=False)
        .head(1)
    ...
    lines = [
        ...
        "",
        "```csv",
        integ.head(50).round(10).to_csv(index=False).strip(),
        "```",
    ]
...
def main() -> None:
    ...
    print(selected[cols].head(35).round(10).to_string(index=False))
