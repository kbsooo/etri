...
import current_0p588_subject_relative_q_postprocess as srq  # noqa: E402
...
KEY = d.KEY
TARGETS = d.TARGETS
BASE_SUB = OUT / "submission_hybrid_0p586_subject_relative_resweep_strict.csv"
BASE_OOF = OUT / "final_hybrid_0p586_subject_relative_resweep_strict_oof.npy"
SUB_OUT = OUT / "submission_hybrid_0p586_subject_relative_resweep_q2loose.csv"
EST_OUT = OUT / "hybrid_0p586_subject_relative_resweep_q2loose_cv_estimate.csv"
OOF_OUT = OUT / "final_hybrid_0p586_subject_relative_resweep_q2loose_oof.npy"

SELECTED = {
    "Q2": "center_shift_p-0.5_w0.2",
}
...
def estimate_candidate(train: pd.DataFrame) -> pd.DataFrame:
    ...
    for target, config_name in SELECTED.items():
        ...
        cfg = srq.parse_config(config_name)
        candidate[:, j] = srq.subject_relative(train, candidate[:, j], cfg)
    candidate = clip(candidate)
    np.save(OOF_OUT, candidate)

    rows = []
    for j, target in enumerate(TARGETS):
        current_loss = srq.loss_col(y[:, j], current[:, j])
        ...
        rows.append(
            {
                ...
                "source": f"subject_relative_{SELECTED[target]}" if target in SELECTED else "unchanged",
            }
        )
...
def make_submission(sub: pd.DataFrame) -> pd.DataFrame:
    ...
    for target, config_name in SELECTED.items():
        cfg = srq.parse_config(config_name)
        out[target] = srq.subject_relative(sub, out[target].to_numpy(dtype=float), cfg)
