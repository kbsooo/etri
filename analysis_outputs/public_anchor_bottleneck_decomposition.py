...
def write_report(known: pd.DataFrame, models: pd.DataFrame, candidates: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"- Candidates outside that MAE band: `{len(resolved)}` / `{len(candidates)}`.",
        f"- Resolved-better candidates versus a2c8: `{len(resolved_better)}`. Resolved-worse candidates: `{len(resolved_worse)}`.",
