...
## Public Mask-Plausibility Split

The ordinal/ambnext public miss (`0.5783033652`, worse than stage2 by `+0.0003583895`) is now a useful negative control: strong OOF on the aggressive q3off/ambnext backbone is not enough. I therefore reweighted the public-subset stress test by mask plausibility instead of treating every simulated mask equally.

```text
scripts                          public_mask_plausibility_reweight.py, public_mask_plausibility_blend_scan.py
mask prior reason                 global prefix/suffix masks require roughly double the all-row probability movement
profile grid                      24 scenario/mask views
best mean-rank                    submission_public_universeens_u65_r04_dc6f3303.csv and u65_r01
u65_r04                           OOF 0.554759, plausibility mean_score 0.576299, six-posterior 0.575013
u65_r01                           OOF 0.554318, plausibility mean_score 0.576335, six-posterior 0.574977
entropy-trusted best              submission_public_minimaxens_r05_a10_h506746.csv, six-posterior 0.5749169278
best compromise blend             submission_public_maskplausens_r05_e8325bb7.csv, OOF 0.555222, plausibility mean_score 0.576278
```

Interpretation: the public inverse branch has two distinct assumptions now, not one. If the public labels are close to the all-row entropy posterior, minimax `r05/r01` remains best. If the public rows are subject/random-like and conservative pseudo-posteriors matter, `u65_r04/u65_r01` are better probes. The new `maskplausens` files are average-case compromises, but they do not win individual profiles, so they are third-line rather than primary.

## Block-Target Public Inverse Dead End

The submission split itself exposes 36 hidden blocks. Since validation-block oracle loss is `0.517878`, I tested an aggressive structural inversion: keep the three public-LB score constraints, but solve for one latent probability per hidden block and target.

```text
script                           public_block_entropy_projection.py
latent space                      36 submission blocks x target masks
constraint residual               0.0
best minimax-prior analogue        0.554332 -> 0.570921
best public2d0-prior analogue      0.561702 -> 0.573676
best proj0-prior analogue          0.562144 -> 0.573923
best stage2-prior analogue         0.567531 -> 0.577868
result                            every tested block-target projection worsens train analogue.
```

Interpretation: the hidden block structure is a real upper-bound clue, but public aggregate scores are too few to allocate probability mass over 252 block-target variables. This rules out a tempting quasi-leakage path and reinforces the current row-cell entropy/minimax choice.

## Boundary-Consensus Dead End

I then tested the less aggressive version of the same hidden-block idea: use only the labels immediately before/after each hidden block. The actual submission has enough exposure for this to be tempting: every target has same-boundary rows, including `Q1 same0=64/same1=33`, `S2 same0=36/same1=86`, and `S3 same0=17/same1=107`.

```text
script                          boundary_consensus_audit.py
best apparent full-OOF move      minimax zeros_only: 0.554332 -> 0.553996
best nested move                 stage2 zeros_only: 0.567531 -> 0.567797
minimax nested zeros_only        0.554332 -> 0.554762
same-boundary nested stage2      0.567531 -> 0.568688
same-or-single nested stage2     0.567531 -> 0.569510
generated score candidates       none
```

Interpretation: endpoint agreement is already partly priced into the temporal/frontier predictions, and the remaining residual is not stable enough to postprocess. The `0.517878` block oracle is still real, but it cannot be approached by boundary labels alone; the missing information must be inside block-level covariates, raw sleep-stage signals, or another label-generation rule.

## Rhythm / Measurement-Process Feature Family

I built `rhythm_regular_features.parquet` with 968 features from sleep-start proxy, quiet-window, and presleep logs. The useful change was not another raw aggregate pass; it was subject-relative rhythm transforms: same-weekend deviations, previous/next day deltas, rolling neighbor deltas, irregularity flags, and light/charge/HR measurement-density shifts.

```text
scripts                         rhythm_regularization_feature_builder.py, rhythm_regularization_scan.py, rhythm_regularization_combo_builder.py
feature rows / features          700 / 968
stage2 base                      0.567531
stage2 non-Q2 strict             0.561317, delta -0.006214, geometry -0.008151, win-rate 1.0
stage2 non-Q2 soft               0.562903, delta -0.004628, geometry -0.005687, win-rate 1.0
q3off650 base                    0.571066
q3off650 non-Q2 strict           0.564012, delta -0.007054, geometry -0.008501, win-rate 1.0
q3off650 non-Q2 soft             0.565892, delta -0.005174, geometry -0.005929, win-rate 1.0
Q2                               no useful rhythm candidate; best candidates select zero blend weight
```

Target-level signals:

```text
Q1  core5h charging minimum zdev                    delta about -0.0050 to -0.0061
Q3  pre6h HR point count same-weekend deviation      delta about -0.0126 to -0.0154
S1  core5h mobile-light minimum subject deviation    delta about -0.0082 to -0.0086
S2  pre3h charging mean prev1 delta                  delta about -0.0051 to -0.0054
S3  pre3h mobile-light max next3 delta               delta about -0.0057 to -0.0064
S4  pre3h mobile-light sum same-weekend deviation    delta about -0.0068 to -0.0076
```

Interpretation: this is a strong structural discovery, but not a direct public submission answer. The Q3 signal is especially revealing: lower-than-usual pre-sleep HR sample count relative to the subject's same weekend/weekday pattern has much higher Q3 positive rate. That points to watch-wear / measurement-density behavior being entangled with the target, not only physiology. S1/S4 are light exposure rhythm deviations, and Q1/S2 are charging rhythm deviations.

Public-risk rejection:

```text
best q3off650 rhythm single     submission_rhythm_q3off650_q3_hr_irregular.csv, scenario score 0.587090
best q1/s2/s3 soft              submission_rhythm_q3off650_q1_s2_s3_soft.csv, scenario score 0.589121
best full non-Q2 soft           submission_rhythm_q3off650_nonq2_soft.csv, scenario score 0.590227
stage2 non-Q2 strict            scenario score 0.599731
current public-safe frontier    u65/minimax scenario scores about 0.5749-0.5763
decision                        keep as feature/label-process evidence; do not promote above u65/minimax public candidates without new public feedback.
```

## Measurement-Process / Wear-Compliance Latents

I separated the measurement-process hypothesis from rhythm features by aggregating raw parquet logs into coverage and gap latents: row/minute observation counts, observation fractions, first/last observed hour, p90/longest gaps, watch-vs-phone coverage, active-sensor count, and subject-relative same-weekend deviations.

```text
scripts                         measurement_process_feature_builder.py, measurement_process_scan.py, measurement_process_combo_builder.py
feature rows / features          700 / 16140
scanned core features            3805
stage2 non-Q2                    0.567531 -> 0.561643, delta -0.005887, geometry -0.004915, win-rate 1.0
stage2 no-Q2/no-Q3               0.567531 -> 0.563438, delta -0.004093, geometry -0.003370, win-rate 1.0
q3off650 non-Q2                  0.571066 -> 0.564592, delta -0.006474, geometry -0.005067, win-rate 1.0
q3off650 no-Q2/no-Q3             0.571066 -> 0.566787, delta -0.004280, geometry -0.003363, win-rate 1.0
Q2                               still closed; no selected feature after target-balanced guard
```

Target-level discoveries:

```text
Q1  core5h usage row count subject-rank: high usage sampling means much lower Q1 positive rate
Q3  pre6h HR observation fraction same-weekend deviation: high HR coverage means much lower Q3
S1  watch-gap irregularity / screen first-observed hour: weak but guarded
S2  all-day watch-light longest gap same-weekend deviation: large gaps lower S2
S3  evening usage sampling p90 gap: large gaps lower S3
S4  pre6h active sensor count same-weekend deviation: more active sensors raise S4
```

Public-risk audit:

```text
best mp single                  submission_mp_q3off650_q3_hr.csv, scenario score 0.587092, mask mean-rank 110.08
best mp no-Q3 soft              submission_mp_q3off650_noq2_noq3_soft.csv, scenario score 0.591289, mask mean-rank 134.75
best public-safe frontier       u65/minimax scenario scores 0.5749-0.5763, mask mean-rank 17.54-35.00
decision                        strong structural clue, rejected as direct public submission.
```

## Q2 Residual Breakthrough Attempt

Q2 resisted raw feature mining, but the temporal diagnosis shows it is the most stateful survey target:

```text
train Q2 mean                    0.562222
subject Q2 rates                 id03 0.818, id08 0.786, id02 0.667 vs id06 0.396, id05 0.432
lag-1 subject autocorr           Q2 about 0.225, highest among targets except Q3 close at 0.219
raw rhythm / measurement Q2      no guarded feature survives
```

The useful reframing was to stop predicting Q2 from sensors and instead test Q2 as a single-column residual from public/orthogonal count families. Results:

```text
stage2 + maskaware Q2            mean -0.001565, Q2 -0.010956, geometry win-rate 1.0
stage2 + orthcap Q2              mean about -0.00104, Q2 about -0.0073, geometry win-rate 1.0
failed ordinal Q2/Q3 public      0.5783033652, worse than stage2 by +0.0003583895
```

The clue is directional: the successful Q2-only swaps lower several high-probability id02/id06 rows and raise id05/id09 rows, while the failed ordinal Q2/Q3 file often moved the opposite way. Q2 appears closer to a subject-state recalibration problem than a direct activity/wear feature problem. The next public probe should isolate this with `submission_q2_stage2safe_r02_369582d6.csv` or the more conservative `submission_q2_stage2safe_r10_a7aa0680.csv`.
