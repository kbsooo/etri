...
import measurement_process_feature_builder as mpb
...
def build_frames() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    mp = broad.prefixed_frame(OUT / "measurement_process_features.parquet", "mp")
    train = train_raw.merge(mp, on=KEY, how="left").sort_values(KEY).reset_index(drop=True)
    sub = sub_raw.merge(mp, on=KEY, how="left").sort_values(KEY).reset_index(drop=True)
    return train_raw, sub_raw, train, sub


def finite_mp_cols(df: pd.DataFrame) -> list[str]:
    context_suffixes = (
        "_dev_subj_med",
        "_zdev_subj_mean",
        "_same_weekend_dev",
    ...
    context_sensor_patterns = (
        "mp__mp_hr_",
        "mp__mp_wlight_",
        "mp__mp_pedo_",
        "mp__mp_watch_",
        "watch_minus_phone",
        "mp__mp_sensor_active",
        "mp__mp_phone_",
    )
    cols = []
    for col in df.columns:
        if not col.startswith("mp__mp_"):
            continue
        has_context = any(suffix in col for suffix in context_suffixes)
        has_core = any(pattern in col for pattern in core_patterns)
        has_context_sensor = any(pattern in col for pattern in context_sensor_patterns)
        if not ((has_core and not has_context) or (has_core and has_context and has_context_sensor)):
            continue
...
def raw_scan(train_feat: pd.DataFrame, base: np.ndarray, targets: list[str], top_n: int) -> pd.DataFrame:
    ...
    cols = finite_mp_cols(train_feat)
...
def guard_and_geometry(
    ...
    candidates = candidates.sort_values(["target", "delta_vs_base", "abs_corr"], ascending=[True, True, False])
    if per_target:
        candidates = candidates.groupby("target", group_keys=False).head(guard_top)
    else:
...
def main() -> None:
    ...
    parser.add_argument("--guard-top", type=int, default=90)
    parser.add_argument("--guard-per-target", action="store_true")
    ...
    guarded = guard_and_geometry(train_raw, sub_raw, train_feat, base, raw, args.guard_top, args.guard_per_target)
    guarded.to_csv(OUT / f"{args.prefix}_guarded.csv", index=False)
    if len(guarded):
        guarded.groupby("target", group_keys=False).head(12).to_csv(OUT / f"{args.prefix}_top_guarded_by_target.csv", index=False)
    ...
    if args.save_strict and len(selected):
        for i, row in selected.head(8).reset_index(drop=True).iterrows():
            if not bool(row["passes_strict"]):
                continue
            feature_slug = str(row["feature"]).replace("mp__mp_", "").replace("__", "_")[:54]
    ...
    print(f"base_loss={base_loss:.9f} features={len(finite_mp_cols(train_feat))} raw_rows={len(raw)} guarded_rows={len(guarded)} selected={len(selected)}")
    if len(guarded):
        show_cols = [
            "target",
            "feature",
        ...
        print(guarded[show_cols].head(40).round(6).to_string(index=False))
    if saved:
