...
Question:
    E366 showed that a human/social row gate can be beaten by a permuted
    target-row gate.  The missing object may not be "richer stories", but a
    row-mask validity latent: which hidden test rows behave like public-good
    movement support and which rows behave like public-bad traps?

JEPA/data2vec translation:
    context = E328/E358 lifestyle state and human/social story features
    target  = known-public row movement support, summarized as same-level
              public-good/public-bad row-mask representations
    action  = use the learned row-mask validity representation to damp/route
              the E365 donor-graft family, then stress it against null masks.

The public scores are used as fixed sensors.  The script does not tune against
new leaderboard feedback and rejects generated candidates when null/permuted
row masks or unstable row-mask targets explain the score.
"""
...
RNG_SEED = 20260531 + 367
ANCHOR_FILE = "submission_e247_featnn1_nn_smooth_sum_top34_f1ff7e86.csv"
UPLOAD_PREFIX = "submission_e367_publicrowmask"

ROWMASK_OUT = OUT / "e367_public_rowmask_validity_rows.csv"
KNOWN_OUT = OUT / "e367_public_rowmask_validity_known.csv"
DIAGNOSTICS_OUT = OUT / "e367_public_rowmask_validity_diagnostics.csv"
STABILITY_OUT = OUT / "e367_public_rowmask_validity_stability.csv"
CANDIDATES_OUT = OUT / "e367_public_rowmask_validity_candidates.csv"
SCORES_OUT = OUT / "e367_public_rowmask_validity_scores.csv"
SCENARIOS_OUT = OUT / "e367_public_rowmask_validity_scenarios.csv"
RANKS_OUT = OUT / "e367_public_rowmask_validity_scenario_ranks.csv"
SUPPORT_OUT = OUT / "e367_public_rowmask_validity_support.csv"
SELECTION_OUT = OUT / "e367_public_rowmask_validity_selection.csv"
REPORT_OUT = OUT / "e367_public_rowmask_validity_report.md"
...
def rowmask_stability(known: pd.DataFrame, known_deltas: dict[str, np.ndarray], full_maps: dict[str, np.ndarray]) -> pd.DataFrame:
    ...
    for name in known[known["available"].fillna(False).astype(bool)]["basename"].astype(str):
        ...
        for key in ["public_row_validity", "public_bad_row_support", "public_validity_Q3", "public_validity_S1"]:
...
def learn_rowmask_latent(
    ...
    rowmask["pred_q3_valid_rank"] = rank01(rowmask["pred_public_validity_Q3"])
...
def generate_candidates(
    ...
    valid = rank01(rowmask["pred_public_row_validity"])
    bad = rank01(rowmask["pred_public_bad_row_support"])
    q3_valid = rank01(rowmask["pred_public_validity_Q3"])
    s1_valid = rank01(rowmask["pred_public_validity_S1"])
    q1_valid = rank01(rowmask["pred_public_validity_Q1"])
    observed_valid = rank01(rowmask["public_row_validity"])
    rng = np.random.default_rng(RNG_SEED)

    def emit_policy(policy: str, family: str, v: np.ndarray, b: np.ndarray, q3v: np.ndarray, s1v: np.ndarray, q1v: np.ndarray, direct: bool = False) -> None:
        good_tail = np.clip((v - 0.62) / 0.38, 0.0, 1.0)
        bad_tail = np.clip((b - 0.62) / 0.38, 0.0, 1.0)
        q3_tail = np.clip((q3v - 0.62) / 0.38, 0.0, 1.0)
        s1_tail = np.clip((s1v - 0.62) / 0.38, 0.0, 1.0)
        q1_tail = np.clip((q1v - 0.62) / 0.38, 0.0, 1.0)
        for amp in [0.97, 1.00, 1.03]:
            if policy == "validboost_baddamp":
                scale = np.clip(0.78 + 0.58 * good_tail - 0.36 * bad_tail, 0.32, 1.42)
                delta = renorm_l1(scale_delta_by_row(base, scale), base, amp)
            elif policy == "hard_public_private":
                scale = np.where(good_tail > 0.55, 1.26, np.where(bad_tail > 0.45, 0.58, 0.92))
                delta = renorm_l1(scale_delta_by_row(base, scale), base, amp)
            elif policy == "target_validity":
                delta = base.copy()
                delta[:, TARGETS.index("Q1")] *= np.clip(0.88 + 0.32 * q1_tail - 0.24 * bad_tail, 0.45, 1.25)
                delta[:, TARGETS.index("Q2")] *= np.clip(0.86 + 0.26 * good_tail - 0.26 * bad_tail, 0.45, 1.20)
                delta[:, TARGETS.index("Q3")] *= np.clip(0.72 + 0.72 * q3_tail - 0.28 * bad_tail, 0.32, 1.38)
                delta[:, TARGETS.index("S1")] *= np.clip(0.72 + 0.78 * s1_tail - 0.28 * bad_tail, 0.32, 1.42)
                delta = renorm_l1(delta, base, amp)
            elif policy == "source_switch":
                delta = base.copy()
                gq3 = q3_tail
                gs1 = s1_tail
                delta[:, TARGETS.index("Q3")] = (1.0 - gq3) * base[:, TARGETS.index("Q3")] + gq3 * q3[:, TARGETS.index("Q3")]
                delta[:, TARGETS.index("S1")] = (1.0 - gs1) * base[:, TARGETS.index("S1")] + gs1 * s1[:, TARGETS.index("S1")]
                row_scale = np.clip(0.86 + 0.36 * good_tail - 0.28 * bad_tail, 0.45, 1.25)
                delta = renorm_l1(scale_delta_by_row(delta, row_scale), base, amp)
            else:
                raise ValueError(policy)
            meta = {
                "variant": f"e367_{'direct_' if direct else ''}{policy}_amp{amp:.2f}",
                "family": family,
                "row_policy": policy,
                "amplitude_scale": amp,
                "gate_mean": float(np.mean(good_tail)),
                "gate_p90": float(np.quantile(good_tail, 0.90)),
    ...
    for policy in ["validboost_baddamp", "hard_public_private", "target_validity", "source_switch"]:
        emit_policy(policy, "learned_public_rowmask_gate", valid, bad, q3_valid, s1_valid, q1_valid)

    # Direct public-row masks are diagnostic controls.  They are not allowed to
    # become the final selection unless a later experiment validates them under
    # stricter public-held-out target reconstruction.
    for policy in ["validboost_baddamp", "target_validity"]:
        emit_policy(policy, "diagnostic_direct_public_rowmask_gate", observed_valid, bad, q3_valid, s1_valid, q1_valid, direct=True)

    null_specs = {
        "inverse_validity": (1.0 - valid, bad, q3_valid, s1_valid, q1_valid),
        "perm_validity_0": (rng.permutation(valid), bad, rng.permutation(q3_valid), rng.permutation(s1_valid), rng.permutation(q1_valid)),
        "perm_validity_1": (rng.permutation(valid), bad, rng.permutation(q3_valid), rng.permutation(s1_valid), rng.permutation(q1_valid)),
        "random_validity": (rng.random(len(valid)), bad, rng.random(len(valid)), rng.random(len(valid)), rng.random(len(valid))),
    }
    for null_id, (v, b, q3v, s1v, q1v) in null_specs.items():
        for policy in ["validboost_baddamp", "target_validity", "source_switch"]:
            good_tail = np.clip((v - 0.62) / 0.38, 0.0, 1.0)
            bad_tail = np.clip((b - 0.62) / 0.38, 0.0, 1.0)
            q3_tail = np.clip((q3v - 0.62) / 0.38, 0.0, 1.0)
            s1_tail = np.clip((s1v - 0.62) / 0.38, 0.0, 1.0)
            ...
            if policy == "validboost_baddamp":
                scale = np.clip(0.78 + 0.58 * good_tail - 0.36 * bad_tail, 0.32, 1.42)
                delta = renorm_l1(scale_delta_by_row(base, scale), base, 1.0)
            elif policy == "target_validity":
                delta = base.copy()
                delta[:, TARGETS.index("Q1")] *= np.clip(0.88 + 0.32 * q1_tail - 0.24 * bad_tail, 0.45, 1.25)
                delta[:, TARGETS.index("Q2")] *= np.clip(0.86 + 0.26 * good_tail - 0.26 * bad_tail, 0.45, 1.20)
                delta[:, TARGETS.index("Q3")] *= np.clip(0.72 + 0.72 * q3_tail - 0.28 * bad_tail, 0.32, 1.38)
                delta[:, TARGETS.index("S1")] *= np.clip(0.72 + 0.78 * s1_tail - 0.28 * bad_tail, 0.32, 1.42)
                delta = renorm_l1(delta, base, 1.0)
            else:
                delta = base.copy()
                delta[:, TARGETS.index("Q3")] = (1.0 - q3_tail) * base[:, TARGETS.index("Q3")] + q3_tail * q3[:, TARGETS.index("Q3")]
                delta[:, TARGETS.index("S1")] = (1.0 - s1_tail) * base[:, TARGETS.index("S1")] + s1_tail * s1[:, TARGETS.index("S1")]
                row_scale = np.clip(0.86 + 0.36 * good_tail - 0.28 * bad_tail, 0.45, 1.25)
                delta = renorm_l1(scale_delta_by_row(delta, row_scale), base, 1.0)
            materialize(
                anchor,
                anchor_logit,
                delta,
                ...
                story_cols,
                {
                    "variant": f"e367_null_{policy}_{null_id}",
                    "family": "null_public_rowmask_gate",
                    "row_policy": policy,
                    "gate_id": null_id,
                    "gate_mean": float(np.mean(good_tail)),
                    "gate_p90": float(np.quantile(good_tail, 0.90)),
...
def public_score_candidates(scored: pd.DataFrame, rowmask: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    ...
    generated["candidate_origin"] = "e367_generated"
    ...
    for model_name in ["ridge_10", "ridge_1", "knn3", "extratrees"]:
        ...
        model.fit(x, y)
        col = f"e367_pred_public_delta_{model_name}"
        pool[col] = np.asarray(model.predict(xp), dtype=np.float64)
        pred_cols.append(col)
    pool["e367_pred_public_delta_mean"] = pool[pred_cols].mean(axis=1)
    pool["e367_pred_public_delta_std"] = pool[pred_cols].std(axis=1).fillna(0.0)
    pool["e367_pred_public_delta_max"] = pool[pred_cols].max(axis=1)
    for col, default in {
    ...
    pool["e367_public_like_score"] = (
        1.10 * good_low(pool["e367_pred_public_delta_mean"])
        + 0.70 * good_low(pool["e367_pred_public_delta_std"])
    ...
    pool = pool.sort_values("e367_public_like_score", ascending=False).reset_index(drop=True)
...
def score_scenario(pool: pd.DataFrame, pred_mean: np.ndarray, pred_std: np.ndarray, base_variant: str) -> pd.DataFrame:
    ...
    if base.empty:
        base = scored[scored["candidate_origin"].astype(str).eq("e364_existing")].sort_values("e367_public_like_score", ascending=False).head(1)
...
def run_scenarios(known: pd.DataFrame, pool: pd.DataFrame, feature_sets: dict[str, list[str]], e365_variant: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for view_name, cols in feature_sets.items():
        for drop_name, drop_base in drops:
            ...
            generated = gated[gated["candidate_origin"].astype(str).eq("e367_generated")].copy()
...
def decide(
    ...
    generated = support[support["candidate_origin"].astype(str).eq("e367_generated")].copy()
    ...
    e365_pls = float(e365_score["e367_public_like_score"].iloc[0]) if len(e365_score) else 0.0
    ...
    real_pls = float(real_score_row["e367_public_like_score"].iloc[0]) if len(real_score_row) else -np.inf

    main_diag = diagnostics[diagnostics["target"].astype(str).eq("public_row_validity")].head(1)
    q3_diag = diagnostics[diagnostics["target"].astype(str).eq("public_validity_Q3")].head(1)
    s1_diag = diagnostics[diagnostics["target"].astype(str).eq("public_validity_S1")].head(1)
    rowmask_predictive_ok = bool(
        len(main_diag)
        and float(main_diag["kfold_spearman"].iloc[0]) > float(main_diag["null_p95"].iloc[0])
        and (
            (len(q3_diag) and float(q3_diag["kfold_spearman"].iloc[0]) > float(q3_diag["null_p95"].iloc[0]))
            or (len(s1_diag) and float(s1_diag["kfold_spearman"].iloc[0]) > float(s1_diag["null_p95"].iloc[0]))
        )
    )
    stability_min = float(stability["public_row_validity_spearman"].min()) if len(stability) else np.nan
    stability_med = float(stability["public_row_validity_spearman"].median()) if len(stability) else np.nan
    rowmask_stable_ok = bool(np.isfinite(stability_min) and stability_min > 0.25 and stability_med > 0.55)
    ...
    if not rowmask_predictive_ok:
        ...
        reason = "The public row-mask target is not predicted by lifestyle/story context beyond permutation nulls; it is not yet a human/social latent."
    elif not rowmask_stable_ok:
        decision = "reject_e367_rowmask_public_target_unstable_keep_e365"
        chosen_variant = e365_variant
        reason = "The row-mask target changes too much when known public observations are dropped; public/private subset identity is not stable enough."
    elif null_veto:
        decision = "reject_e367_null_rowmask_wins_keep_e365"
        chosen_variant = e365_variant
        reason = "A null/permuted row-mask gate beats the learned row-mask gates under jackknife; keep E365."
    elif len(best_real) and real_top1 >= max(0.20, e365_top1 + 0.10) and real_pls >= e365_pls - 0.05:
        decision = "select_e367_public_rowmask_replacement"
        chosen_variant = best_real_variant
        reason = "The learned public/private row-mask gate beats E365 and null controls under jackknife stress."
    elif len(best_real) and real_top10 >= e365_top10 + 0.10 and real_pls >= e365_pls + 0.10:
        decision = "select_e367_public_rowmask_stable_alt"
        chosen_variant = best_real_variant
        reason = "The learned public/private row-mask gate is more stable top10 and stronger on public-like score than E365."
    else:
        decision = "keep_e365_no_e367_uplift"
        chosen_variant = e365_variant
        reason = "The row-mask latent is diagnostic, but generated candidates do not beat E365 strongly enough."
...
def write_report(
    ...
    scenario_summary = (
        ...
        .agg(
            ...
            top_generated_rate=("top_origin", lambda x: float(np.mean(pd.Series(x).astype(str).eq("e367_generated")))),
    ...
    score_cols = [
        ...
        "family",
        "candidate_origin",
        "e367_public_like_score",
        "e367_pred_public_delta_mean",
        "e367_pred_public_delta_std",
        "public_bad_axis_sum",
        "rowstate_pred_public_loss_mean",
        "rowmask_wmean_pred_public_row_validity",
        "rowmask_wmean_pred_public_bad_row_support",
...
def main() -> None:
    ...
    print(selected[["decision", "variant", "family", "candidate_origin", "selected_uploadsafe_file", "rowmask_predictive_ok", "rowmask_stable_ok", "best_real_top1_rate", "best_null_top1_rate"]].round(6).to_string(index=False))
