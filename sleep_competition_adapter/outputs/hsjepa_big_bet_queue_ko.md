...
| --- | --- | --- | --- | ---: | --- |
| `Action Decoder Ablation Suite` | The next breakthrough comes from choosing the correct action-decoder order, not from adding more latent features. | Compare toxicity-first, support-first, and route-first decoders under one LB-sensor priority table. | `action_decoder_ablation_ready_route_frontier_leads` | `-0.0025` | Public LB contradicts the top-ranked decoder order, or route-first gains vanish under stronger null matching. |
