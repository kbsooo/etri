...
| --- | --- | --- | --- | ---: | --- |
| `OG-only Human-State Assignment Teacher` | The public-sensor teacher can be replaced by personal/cohort/time human-state consistency. | Train a row-target support teacher from OG personal/cohort/time masks, then feed it into the existing invariant decoder. | `row_support_is_primary_bottleneck` | `-0.003` | Portable human context cannot localize support rows; row-oracle + target prior remains far above all current OG scores. |
