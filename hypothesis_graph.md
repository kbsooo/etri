...
- 상태: 강한 관찰 증거 있음; predictive utility 미검증.
...
- 관측: E49 found largest target movements `Q3 = 0.011540`, `Q1 = 0.010345`, `S3 = 0.009688`; `Q1` and `S1` are worse under all simple prior proxies; movement is subject-concentrated (`id05/id09/id08/id03/id01` top); top high-movement calendar masks are `gap_adjacent` or `between_train_runs`.
- 성공/폐기 기준: strengthened if calendar-mask features reduce selector error below mixmin-a2c8 edge scale or identify a stable mixmin-relative low-energy subset; weakened if calendar features only restate target-level movement and fail anchor order.
- public LB 기대 반응: if true, the next public-improving candidate should not be a global prior tweak; it should preserve mixmin's anchor-loss direction while changing probabilities mostly on specific subject-calendar hidden blocks.
- 제출 전략: no immediate submission. Build a mixmin-relative candidate only after calendar-block selector stress survives known anchors and private-risk diagnostics.
...
E49는 H48을 추가한다. Mixmin의 target movement는 `Q3/Q1/S3`에 크고, `Q1/S1`은 simple prior proxy에서는 오히려 불리하다. Train/test는 각 subject timeline 안의 hidden calendar blocks 형태이며, high-movement blocks는 gap-adjacent/between-train-runs 형태가 많다. 이 관찰은 "public subset이 단순 mask다"라는 H44와 다르다. H44는 public LB 평가 subset 복원을 묻고 반증됐고, H48은 데이터 생성 자체가 subject-calendar mask restoration이라는 가설이다. 다음 실험은 H48을 selector/representation으로 변환해야 한다.
