...
E67은 H64를 절반만 살렸다. First-order anchor-tail gate는 Q2/S3 add-back을 무작정 막는 것보다 낫다. `tail_meanneg_m1.00`는 `-6.933e-6`까지 내려갔고, `tail_p90_nonpos_m1.00`는 matched base를 `432/432` 이기면서 그중 `360/432`은 max-set tail도 중립이었다. 하지만 margin gate는 여전히 `0`이다. 따라서 Q2/S3 tail-risk translator는 살아있는 방향이지만, 현재 형태는 known-anchor tail derivative에 의존한 sub-margin micro-edge다. 다음 질문은 이 tail-gated cell들이 non-anchor hidden/block/row-calibration stress에서도 살아남는지다.
