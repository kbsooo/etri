...
E53은 calendar-flank block-rate/count branch의 가장 단순한 count-state posterior를 반증했다. Same-subject를 허용한 `calendar_count_local`은 pseudo-hidden에서 subject mean 대비 `-0.005266` 좋아졌지만, private-safe에 가까운 `calendar_count_strict`는 `+0.001434` 나빠졌다. 실제 hidden block에서는 strict rate가 mixmin을 약하게 선호했지만 weighted delta는 `-0.000179`뿐이고 target alignment가 S3/S2/Q2에만 좋고 S1/Q1/Q3/S4에는 불리했다. 따라서 calendar flanks는 버리지 않지만, simple count posterior는 후보 생성기가 아니라 LeJEPA-style energy다. 다음 live branch는 더 풍부한 raw overnight context, Q/S target-dependency count manifold, 또는 mixmin-hard world generation이다.
