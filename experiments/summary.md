...
1. Build the channel-patch MoE decoder around fold-local subject-relative expert latents, not PCA-compressed day CLS. Use event/body/phone/mobility/physio/behavior/all-groups experts as separate inputs and learn target-wise gates with nested selection.
   - Success criterion: beat the no-PCA fold-local selected decoder (`0.617748`) without increasing v83 drift beyond the current `0.076986` band.
