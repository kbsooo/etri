...
1. Build latent-level channel MoE around fold-local subject-relative expert latents, not PCA-compressed day CLS and not only prediction stacking. Use event/body/phone/mobility/physio/behavior/all-groups experts as separate inputs and learn target-wise gates before the residual decoder.
   - Success criterion: beat the prediction-level hybrid (`0.616459`) without increasing v83 drift beyond the current `0.077083` band.
