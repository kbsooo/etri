...
E49 makes the next feature policy narrower: start from subject-calendar mask context, but treat it as JEPA context/energy, not as a raw row-order feature dump. E50 makes it narrower again: calendar movement is not enough to rank public candidates because it fails to predict mixmin as best. A useful feature must answer why mixmin moved Q1/S1 against simple priors, why high-movement rows cluster in specific calendar holes, and why anchor-loss/binary-world geometry can see a public edge that calendar fingerprints alone miss.
