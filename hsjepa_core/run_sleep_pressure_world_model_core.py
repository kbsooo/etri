#!/usr/bin/env python3
"""HS-JEPA sleep-pressure world-model core experiment.

This is a public-free core experiment.  It asks whether visible daily
human-life context can predict a hidden sleep-pressure representation before
any competition adapter touches Q/S labels.

The hidden target is label-free.  It is built from subject-relative night
disturbance, physiological load, routine rhythm, and environment stability
surrogates.
"""
...
OUT_DIR = ROOT / "hsjepa_core" / "outputs" / "sleep_pressure_world_model_core"
PAPER_DOC = ROOT / "paper_hsjepa_core" / "SLEEP_PRESSURE_WORLD_MODEL_CORE_KO.md"
...
def matching_columns(raw_cols: list[str], patterns: tuple[str, ...], exact: tuple[str, ...] = ()) -> list[str]:
    selected = []
    for col in raw_cols:
        if col in exact or any(pattern in col for pattern in patterns):
            selected.append(col)
    return sorted(set(selected))


def sleep_pressure_families(raw_cols: list[str]) -> dict[str, list[str]]:
    """Human-theory feature families used as label-free hidden targets."""

    return {
        "night_disturbance": matching_columns(
            raw_cols,
            (
                "_night_value",
                "night_usage_",
            ),
            ("night_step_sum", "hr_night_mean"),
        ),
        "physiological_load": matching_columns(
            raw_cols,
            (
                "step_sum",
                "distance_sum",
        ...
        "social_cognitive_arousal": matching_columns(
            raw_cols,
            (
                "usage_social",
                "usage_call",
                "usage_finance",
                "usage_religion_ritual",
                "usage_media",
                "usage_search_browser",
                "usage_total",
                "usage_entropy",
                "screen_use_",
                "phone_activity_",
        ...
        "rest_environment_stability": matching_columns(
            raw_cols,
            (
                "gps_",
                "wifi_",
        ...
        "calendar_routine_pressure": matching_columns(
            raw_cols,
            (),
            (
                "dayofweek",
...
def build_sleep_pressure_descriptors(
    frame: pd.DataFrame,
    raw_cols: list[str],
    ...
    for indices in ordered_subject_indices(frame):
        prev_sub, base_sub = rolling_baseline(rel_values[indices], window=5)
    ...
    family_descriptor_cols: dict[str, list[str]] = {}
    for family, cols in families.items():
        positions = [col_pos[col] for col in cols if col in col_pos]
        if len(positions) < 2:
            continue
        ...
        for suffix, arr in arrays.items():
            for offset, source_col in enumerate([raw_cols[pos] for pos in positions]):
                col = f"sp_{family}_{suffix}_{source_col}"
                descriptor_data[col] = arr[:, offset]
                family_cols.append(col)

        descriptor_data[f"sp_intensity_{family}"] = np.sqrt(np.nanmean(np.square(residual[:, positions]), axis=1))
        descriptor_data[f"sp_jump_{family}"] = np.sqrt(np.nanmean(np.square(delta[:, positions]), axis=1))
        descriptor_data[f"sp_abs_state_{family}"] = np.sqrt(np.nanmean(np.square(rel_values[:, positions]), axis=1))
        descriptor_data[f"sp_recovery_debt_{family}"] = np.nanmean(abs_residual[:, positions] + 0.5 * abs_delta[:, positions], axis=1)
        family_cols.extend(
            [
                f"sp_intensity_{family}",
                f"sp_jump_{family}",
                f"sp_abs_state_{family}",
                f"sp_recovery_debt_{family}",
            ]
        )
        family_descriptor_cols[family] = family_cols

    descriptor = pd.DataFrame(descriptor_data, index=frame.index)
    intensity_cols = [c for c in descriptor.columns if c.startswith("sp_intensity_")]
    jump_cols = [c for c in descriptor.columns if c.startswith("sp_jump_")]
    debt_cols = [c for c in descriptor.columns if c.startswith("sp_recovery_debt_")]
    descriptor["sp_intensity_mean"] = descriptor[intensity_cols].mean(axis=1)
    descriptor["sp_jump_mean"] = descriptor[jump_cols].mean(axis=1)
    descriptor["sp_recovery_debt_mean"] = descriptor[debt_cols].mean(axis=1)
    return descriptor, family_descriptor_cols


def masked_sleep_pressure_state(
    frame: pd.DataFrame,
    ...
    for family, cols in families.items():
        target_cols = family_descriptor_cols.get(family, [])
        context_cols = [col for col in raw_cols if col not in set(cols)]
        if len(target_cols) < 4 or len(context_cols) < 2:
            continue
        target_state, _ = encode_latent(descriptors, target_cols, CONFIG.latent_dims_per_view, RANDOM_SEED)
        pred, null_pred = ridge_predict_oof(relative_frame, context_cols, target_state, groups, CONFIG)
        residual = target_state - pred
        energy = np.sqrt(np.mean(np.square(residual), axis=1))

        for comp in range(target_state.shape[1]):
            state[f"spwm_pred_{family}_c{comp + 1}"] = pred[:, comp]
            state[f"spwm_resid_{family}_c{comp + 1}"] = residual[:, comp]
        state[f"spwm_energy_{family}"] = energy
        state[f"spwm_energy_rank_{family}"] = rank01(energy)
        state[f"spwm_observed_intensity_{family}"] = descriptors[f"sp_intensity_{family}"].to_numpy(dtype=np.float64)
        state[f"spwm_observed_jump_{family}"] = descriptors[f"sp_jump_{family}"].to_numpy(dtype=np.float64)
        state[f"spwm_observed_recovery_debt_{family}"] = descriptors[f"sp_recovery_debt_{family}"].to_numpy(dtype=np.float64)

        rows.append(
            {
                "task": "masked_context_to_sleep_pressure_family",
                "target": family,
...
def future_sleep_pressure_state(
    frame: pd.DataFrame,
    relative_frame: pd.DataFrame,
    raw_cols: list[str],
    descriptors: pd.DataFrame,
    ...
    all_descriptor_cols = sorted({col for cols in family_descriptor_cols.values() for col in cols})
    target_basis, _ = encode_latent(descriptors, all_descriptor_cols, CONFIG.future_state_dims, RANDOM_SEED)
    ...
    target_state = target_basis[next_row[valid].astype(int)]
    ...
    for comp in range(target_state.shape[1]):
        state[f"spwm_future_pred_c{comp + 1}"] = pred[:, comp]
        state[f"spwm_future_resid_c{comp + 1}"] = residual[:, comp]
    state["spwm_future_energy"] = energy
    state["spwm_future_energy_rank"] = rank01(energy)
    metrics = pd.DataFrame(
        [
            {
                "task": "current_context_to_next_sleep_pressure",
                "target": "next_sleep_pressure_state",
                "components": int(target_state.shape[1]),
...
def build_sleep_pressure_world_state(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, list[str]]]:
    views = make_views(frame)
    raw_cols = sorted({col for view in views for col in view.columns})
    families = sleep_pressure_families(raw_cols)
    families = {key: value for key, value in families.items() if len(value) >= 2}
    relative_frame = subject_relative_frame(frame, raw_cols)
    descriptors, family_descriptor_cols = build_sleep_pressure_descriptors(frame, raw_cols, families)
    masked_state, masked_metrics = masked_sleep_pressure_state(
        frame,
        relative_frame,
        raw_cols,
        families,
        descriptors,
        family_descriptor_cols,
    )
    future_state, future_metrics = future_sleep_pressure_state(
        frame,
        relative_frame,
        raw_cols,
        descriptors,
    ...
    pred_cols = [c for c in state.columns if c.startswith("spwm_pred_") or c.startswith("spwm_future_pred_")]
    resid_cols = [c for c in state.columns if c.startswith("spwm_resid_") or c.startswith("spwm_future_resid_")]
    observed_cols = [c for c in state.columns if c.startswith("spwm_observed_")]
    state["spwm_energy_mean"] = state[energy_cols].mean(axis=1)
    state["spwm_energy_max"] = state[energy_cols].max(axis=1)
    state["spwm_energy_rank_mean"] = state[rank_cols].mean(axis=1)
    state["spwm_energy_rank_max"] = state[rank_cols].max(axis=1)
    state["spwm_observed_intensity_mean"] = state[[c for c in observed_cols if "intensity" in c]].mean(axis=1)
    state["spwm_observed_jump_mean"] = state[[c for c in observed_cols if "jump" in c]].mean(axis=1)
    state["spwm_observed_recovery_debt_mean"] = state[[c for c in observed_cols if "recovery_debt" in c]].mean(axis=1)
    metrics = pd.concat([masked_metrics, future_metrics], ignore_index=True, sort=False)
    colsets = {
        "sleep_pressure_predicted": pred_cols,
        "sleep_pressure_energy": energy_cols
        + rank_cols
        + ["spwm_energy_mean", "spwm_energy_max", "spwm_energy_rank_mean", "spwm_energy_rank_max"],
        "sleep_pressure_observed": observed_cols
        + [
            "spwm_observed_intensity_mean",
            "spwm_observed_jump_mean",
            "spwm_observed_recovery_debt_mean",
        ],
        "sleep_pressure_full": sorted(set(pred_cols + resid_cols + observed_cols + list(state.columns))),
    }
...
def summarize(
    ...
    predicted = subject_all[subject_all["feature_set"].eq("sleep_pressure_predicted_calibrated10")]
    full = subject_all[subject_all["feature_set"].eq("sleep_pressure_full_calibrated10")]
    ...
    return {
        "package": "sleep_pressure_world_model_core",
        "status": "sleep_pressure_world_model_core_ready",
        "uses_public_score_ledger": False,
        "uses_prior_submission_probabilities": False,
        "uses_proprietary_embedding_api": False,
        "uses_label_as_pretext_target": False,
        "hidden_target": "label_free_sleep_pressure_surrogate",
        ...
        "sleep_pressure_predicted_calibrated_logloss": None
        if predicted.empty
        else float(predicted["logloss"].iloc[0]),
        "sleep_pressure_full_calibrated_logloss": None if full.empty else float(full["logloss"].iloc[0]),
        "sleep_pressure_predicted_delta_vs_prior": None
        if prior.empty or predicted.empty
        else float(predicted["logloss"].iloc[0] - prior["logloss"].iloc[0]),
        "sleep_pressure_full_delta_vs_prior": None
...
def build_markdown(
    ...
    predicted_delta = summary.get("sleep_pressure_predicted_delta_vs_prior")
    full_delta = summary.get("sleep_pressure_full_delta_vs_prior")
    verdict = "core_positive" if (predicted_delta is not None and predicted_delta < 0) or (full_delta is not None and full_delta < 0) else "core_mixed_or_negative"
    subject_all = (
    ...
    return f"""# Sleep-Pressure World Model Core
...
def main() -> None:
    ...
    views = make_views(frame)
    raw_cols = sorted({col for view in views for col in view.columns})
    state, pretext_metrics, colsets = build_sleep_pressure_world_state(frame)
    state = pd.concat(
    ...
    feature_sets = {
        ...
        "sleep_pressure_energy": colsets["sleep_pressure_energy"],
        "sleep_pressure_observed": colsets["sleep_pressure_observed"],
        "sleep_pressure_full": colsets["sleep_pressure_full"],
        "raw_plus_sleep_pressure_full": sorted(set(raw_cols + colsets["sleep_pressure_full"])),
    }
    ...
    leakage = subject_leakage_probe(
        train_frame,
        train_features,
        {key: value for key, value in feature_sets.items() if key not in {"prior_only", "raw_plus_sleep_pressure_full"}},
    )
    ...
    submission = build_downstream_probe_submission(
        ...
        train_features,
        test_features,
        colsets["sleep_pressure_predicted"],
    )
    candidate_name = f"submission_hsjepa_sleep_pressure_world_model_probe_{short_hash(submission)}_uploadsafe.csv"
    ...
    state.to_csv(OUT_DIR / "sleep_pressure_world_model_state.csv", index=False)
    pretext_metrics.to_csv(OUT_DIR / "sleep_pressure_world_model_pretext_metrics.csv", index=False)
    probe_metrics.to_csv(OUT_DIR / "sleep_pressure_world_model_probe_metrics.csv", index=False)
    probe_predictions.to_csv(OUT_DIR / "sleep_pressure_world_model_probe_predictions.csv", index=False)
    nn_metrics.to_csv(OUT_DIR / "sleep_pressure_world_model_neighbor_consistency.csv", index=False)
    leakage.to_csv(OUT_DIR / "sleep_pressure_world_model_subject_leakage.csv", index=False)
    (OUT_DIR / "sleep_pressure_world_model_summary.json").write_text(
        json.dumps(summary, indent=2, ensure_ascii=False, allow_nan=False),
        encoding="utf-8",
    )
    markdown = build_markdown(summary, pretext_metrics, probe_metrics, nn_metrics, leakage, candidate_name)
    (OUT_DIR / "SLEEP_PRESSURE_WORLD_MODEL_CORE_KO.md").write_text(markdown, encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "sleep_pressure_predicted_delta_vs_prior": summary["sleep_pressure_predicted_delta_vs_prior"],
                "sleep_pressure_full_delta_vs_prior": summary["sleep_pressure_full_delta_vs_prior"],
