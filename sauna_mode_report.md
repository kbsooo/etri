...
## E213 업데이트: JEPA 축이 랜덤 cherry-pick인지 찔렀다

내가 발견한 가장 이상한 점:

`JEPA가 Q3/S4에 좁은 신호를 준다는 결론은 좋지만, 많은 latent coordinate 중 하나를 고른 것일 수 있다. 이게 맞다면 permutation이나 같은 PC-family 안에서 비슷한 축이 쉽게 나와야 한다.`

실험:

- `analysis_outputs/e213_jepa_axis_specificity_audit.py`
- report: `analysis_outputs/e213_jepa_axis_specificity_audit_report.md`

실험 결과:

- Q3 `e208_resid_self_pc10`: delta `-0.005775`, half win `0.950000`, global/subject permutation p `0.020408`, same-family pool rank `1/16`.
- S4 `e208_pred_pc14`: delta `-0.003134`, half win `0.733333`, global/subject permutation p `0.020408`, same-family pool rank `1/16`.
- S4의 같은-family runner-up은 오히려 adverse `+0.001377`라서 S4 pc14가 특히 특이하다.

생각이 어떻게 바뀌었는지:

`E211이 틀린다면 "JEPA 축이 가짜였다"보다는 "진짜 JEPA 축을 public-stable probability로 번역하는 데 실패했다"가 우선 해석이다. 병목은 representation 발견보다 calibration/public-tail translation 쪽이다.`

다음으로 가장 정보량이 큰 행동:

E211 제출 우선순위는 유지한다. bad feedback이 오면 JEPA를 버리는 게 아니라, Q3/S4 axis는 살려두고 anchor/gate/translation을 바꿔야 한다.

## E214 업데이트: JEPA benefit gate는 답이 아니었다

내가 발견한 가장 이상한 점:

`JEPA 축은 진짜처럼 보이는데, 그 축을 어느 row에서 믿을지 학습하는 작은 classifier는 충분히 강하지 않았다.`

실험:

- `analysis_outputs/e214_jepa_benefit_gate_translation.py`
...
- best benefit-gated policy `q3raw_s4benefit_rank` delta: `-0.000918`.
- geometry는 일부 좋아졌다. `q3raw_s4benefit_rank` geometry `-0.000987`.
- 하지만 local signal을 너무 많이 버려서 selected submission은 없다.

생각이 어떻게 바뀌었는지:

`JEPA 병목은 "row별로 keep/drop을 분류하면 해결"되는 단순한 문제가 아니다. benefit gate에는 약한 정보가 있지만 frontier를 밀 정도는 아니다.`

다음으로 가장 정보량이 큰 행동:

E214는 제출하지 않는다. E211을 먼저 public 센서로 써야 한다. E211이 실패하면 같은 step 위에 gate를 더 얹는 게 아니라, JEPA target representation 자체나 public-tail translation 법칙을 바꿔야 한다.

## E215/E216 업데이트: JEPA target을 바꾸자 다른 신호가 나왔다

내가 발견한 가장 이상한 점:

`E208 JEPA는 Q3/S4를 말했는데, masked-family JEPA는 Q1/S2/S4를 말했다. JEPA가 안 먹힌 게 아니라, 어떤 representation을 target으로 삼느냐에 따라 보이는 target 세계가 달라진다.`

실험:

- E215: `analysis_outputs/e215_masked_family_jepa_probe.py`
- E216: `analysis_outputs/e216_masked_family_jepa_materialization.py`
...
- E215 geometry pass count: `10`
- E216 local best `q1_s2_s4_rank`: delta `-0.001807`, geometry `-0.001628`, subject-half win `1.000`
- 그런데 frontier stress는 이 broad combo를 거부했다.
- 살아남은 것은 S2-only:
...
`JEPA를 더 써볼 가치는 있다. 단, 같은 E208 축에 gate를 더 얹는 방향이 아니라, target representation을 바꾸는 방향이 맞다. 하지만 public-safe translation은 여전히 아주 좁다. E215의 넓은 Q1/S2/S4 신호 중 public stress를 통과한 것은 S2뿐이다.`

다음으로 가장 정보량이 큰 행동:

첫 JEPA 제출은 여전히 E211이 맞다. 하지만 두 번째 JEPA 센서가 필요하면 E216 S2-only가 가장 깔끔하다. 이 결과는 0.54 병목이 capacity 부족이 아니라 `representation target × target-specific public-tail translation` 문제라는 쪽으로 더 기운다.
