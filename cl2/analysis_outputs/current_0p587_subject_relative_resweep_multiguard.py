...
import current_0p587_subject_relative_resweep as rs
...
def selected_configs() -> pd.DataFrame:
    top = pd.read_csv(OUT / "current_0p587_subject_relative_resweep_results.csv")
    top = top[top["delta_vs_base"] < -0.00008].copy()
    return top.sort_values(["target", "blend_loss"]).groupby("target").head(6).reset_index(drop=True)
...
def repeated_subject_half(train: pd.DataFrame, current: np.ndarray, selected: pd.DataFrame) -> pd.DataFrame:
    ...
    detail.to_csv(OUT / "current_0p587_subject_relative_resweep_multiguard_subject_detail.csv", index=False)
    return (
...
def main() -> None:
    ...
    selected.to_csv(OUT / "current_0p587_subject_relative_resweep_multiguard_candidates.csv", index=False)
    split = repeated_subject_half(train, current, selected)
    split.to_csv(OUT / "current_0p587_subject_relative_resweep_multiguard_subject.csv", index=False)
    geo = geometry_guardrail(train, sub, current, selected)
    geo.to_csv(OUT / "current_0p587_subject_relative_resweep_multiguard_geometry.csv", index=False)
    merged = (
        selected.merge(split, on=["target", "config"], how="left")
        .merge(geo, on=["target", "config"], how="left")
        .sort_values(["target", "delta_vs_base"])
    )
    merged["passes_loose"] = (
    ...
    merged.to_csv(OUT / "current_0p587_subject_relative_resweep_multiguard_merged.csv", index=False)
    print(
        merged[
            [
                "target",
