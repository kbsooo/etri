...
VARIANTS = {
    "activation_latency": ["first_", "latency", "recency", "phone_before_body", "body_before_phone"],
    "activation_slope": ["activation_slope", "step_slope", "light_slope", "activation_score", "clean_start"],
    "phone_inertia": ["phone_still", "phone_before", "screen_before", "inertia"],
    "light_hr_entrainment": ["bright", "light", "hr_", "clean_start"],
    "sleep_recovery_interaction": ["sleep_amount", "sleep_eff", "fragmented_sleep", "short_sleep", "long_sleep"],
...
def select_columns(columns: list[str], patterns: list[str]) -> list[str]:
    if "__BASE_VALUES_ONLY__" in patterns:
        ...
        return sorted([col for col in columns if col.startswith("z_wai_") and not any(token in col for token in derived)])
    return sorted([col for col in columns if col.startswith("z_wai_") and any(pattern in col for pattern in patterns)])
...
def run(args: argparse.Namespace) -> None:
    ...
    for name, patterns in VARIANTS.items():
        ...
        raw_path = output_dir / f"domain_wake_activation_inertia_{name}_v1.parquet"
        raw_report = {
            "name": f"Wake Activation Inertia Variant: {name}",
            "variant": name,
            "kind": "raw",
            "patterns": patterns,
            "output": str(raw_path),
        ...
        additive_path = output_dir / f"domain_best_plus_wake_activation_inertia_{name}_v1.parquet"
        additive_report = {
            "name": f"Best Plus Wake Activation Inertia Variant: {name}",
            "variant": name,
            "kind": "best_plus",
            "patterns": patterns,
    ...
    report_path = output_dir / "domain_wake_activation_inertia_variants_v1.report.json"
    ...
    report_path.with_suffix(".md").write_text(
        "# Wake Activation Inertia Variants\n\n"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build pruned wake activation inertia variants and best-plus variants.")
    parser.add_argument("--input", default="artifacts/domain_wake_activation_inertia_v1.parquet")
