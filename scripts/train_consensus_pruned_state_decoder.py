...
FIXED_MAPS: dict[str, dict[str, CandidateSpec]] = {
    ...
    "consensus_signal": {
        ...
        "S4": CandidateSpec("only_cross_modal", "hgb", 0.10),
    },
    "consensus_s4_temporal": {
        "Q1": CandidateSpec("only_rhythm", "hgb", 0.20),
        "Q2": CandidateSpec("no_ratio", "state_mean", 0.35),
        "Q3": CandidateSpec("no_sleep", "hgb", 0.35),
        "S1": CandidateSpec("only_rhythm", "hgb", 0.20),
...
def run_fixed_map(
    ...
    for fold_i, fold in enumerate(folds):
        ...
        for target_i, target in enumerate(TARGET_COLUMNS):
            ...
            fold_rows.append({"variant": name, "fold": fold_i, "target": target, "source": spec.name})
    ...
    return oof, sample_pred, {"variant": name, "avg_log_loss": avg, **per_target, "fold_sources": fold_rows}
...
def main() -> None:
    ...
    used_presets = sorted(
        {
            ...
            for spec in fixed_map.values()
            if spec.preset != "subject_prior"
        }
    )
    preset_columns = {
    ...
    for name, fixed_map in FIXED_MAPS.items():
        oof, submission, report = run_fixed_map(name, fixed_map, train, sample, train_x, sample_x, preset_columns, args)
        reports.append({k: v for k, v in report.items() if k != "fold_sources"})
        all_fold_rows.extend(report["fold_sources"])
        ...
        reports[-1]["drift_vs_reference"] = drift.get("mean_abs_drift")
        reports[-1]["corr_vs_reference"] = drift.get("corr")

    report_df = pd.DataFrame(reports).sort_values("avg_log_loss")
    report_df.to_csv(output_dir / "fixed_consensus_scores.csv", index=False)
    pd.DataFrame(all_fold_rows).to_csv(output_dir / "fixed_sources.csv", index=False)
    (output_dir / "report.json").write_text(
        json.dumps(
            {
                "scores": report_df.to_dict(orient="records"),
                "fixed_maps": {
                    name: {target: spec.name for target, spec in fixed_map.items()}
                    for name, fixed_map in FIXED_MAPS.items()
                },
                "args": vars(args),
            },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fixed consensus target maps for pruned state decoders.")
    parser.add_argument("--train-path", default="data/ch2026_metrics_train.csv")
    parser.add_argument("--sample-path", default="data/ch2026_submission_sample.csv")
    parser.add_argument("--output-dir", default="outputs/fixed_consensus_pruned_state_decoder_v1")
