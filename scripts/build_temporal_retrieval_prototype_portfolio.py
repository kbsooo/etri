...
def main() -> None:
    output_dir = Path("outputs/temporal_retrieval_prototype_portfolio")
    ...
    base_oof_frame = normalize_keys(pd.read_csv("outputs/lb_feedback_recovery_uploads/oof_15_v18_old15_prob_blend.csv"))
    base_sub_frame = normalize_keys(pd.read_csv("outputs/lb_feedback_recovery_uploads/submission_15_v18_old15_prob_blend.csv"))
    source_oof_frame = normalize_keys(pd.read_csv("outputs/temporal_retrieval_prototype_meta/oof_lgbm_temporal_retrieval_prototype_meta.csv"))
    source_sub_frame = normalize_keys(pd.read_csv("outputs/temporal_retrieval_prototype_meta/submission_lgbm_temporal_retrieval_prototype_meta.csv"))
    ...
    variants = [
        (
            "trp_s2tail_w100_q3midtail_w035",
            [
                Move("S2", 0.8, 1.000001, 1.00, "lgbm_trp_meta"),
                Move("Q3", 1 / 3, 1.000001, 0.35, "lgbm_trp_meta"),
            ],
        ),
        (
            "trp_s2tail_w100_q3midtail_w050",
            [
                Move("S2", 0.8, 1.000001, 1.00, "lgbm_trp_meta"),
                Move("Q3", 1 / 3, 1.000001, 0.50, "lgbm_trp_meta"),
            ],
        ),
        (
            "trp_s2tail_w080_q3midtail_w035",
            [
                Move("S2", 0.8, 1.000001, 0.80, "lgbm_trp_meta"),
                Move("Q3", 1 / 3, 1.000001, 0.35, "lgbm_trp_meta"),
            ],
        ),
    ]
    ...
    (output_dir / "portfolio_report.json").write_text(json.dumps(report.to_dict(orient="records"), ensure_ascii=False, indent=2), encoding="utf-8")
