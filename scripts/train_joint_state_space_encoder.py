...
def residual_neighborhood_targets(
    residual_targets: np.ndarray,
    panel_targets: np.ndarray,
    k: int = 31,
) -> tuple[np.ndarray, dict[str, int | float]]:
    ...
    scaler = StandardScaler()
    scaled = scaler.fit_transform(raw).astype(np.float32)
    d2 = ((scaled[:, None, :] - scaled[None, :, :]) ** 2).mean(axis=2)
    np.fill_diagonal(d2, np.inf)
    kk = max(2, min(k, len(scaled) - 1))
    ...
    scale = np.maximum(np.median(chosen, axis=1, keepdims=True), 1e-6)
    weights = np.exp(-chosen / scale)
    weights = weights / np.maximum(weights.sum(axis=1, keepdims=True), 1e-12)
    neighbor_residual = residual[idx]
    neighbor_panel = panel[idx]
    mean_residual = (weights[:, :, None] * neighbor_residual).sum(axis=1)
    mean_panel = (weights[:, :, None] * neighbor_panel).sum(axis=1)
    std_residual = np.sqrt(np.maximum((weights[:, :, None] * (neighbor_residual - mean_residual[:, None, :]) ** 2).sum(axis=1), 0.0))
    std_panel = np.sqrt(np.maximum((weights[:, :, None] * (neighbor_panel - mean_panel[:, None, :]) ** 2).sum(axis=1), 0.0))
    density = np.column_stack(
        [
            np.min(chosen, axis=1),
            np.mean(chosen, axis=1),
            np.median(chosen, axis=1),
            np.max(chosen, axis=1),
            chosen[:, min(4, kk - 1)],
            chosen[:, -1] - chosen[:, 0],
        ]
    )
    self_delta = np.hstack([residual - mean_residual, panel - mean_panel])
    targets = np.hstack([mean_residual, mean_panel, std_residual, std_panel, self_delta, density])
    targets = np.clip(np.nan_to_num(targets, nan=0.0, posinf=8.0, neginf=-8.0), -8.0, 8.0)
    meta = {
        "residual_neighborhood_k": int(kk),
        "residual_neighborhood_target_dim": int(targets.shape[1]),
        "residual_neighborhood_mean_distance": float(np.mean(chosen)),
    }
    return targets.astype(np.float32), meta
...
def fit_view_context_window_decoder(
    method: str,
    fit_view_preds: list[np.ndarray],
    y_fit: np.ndarray,
    base_fit: np.ndarray,
    ...
    apply_pred = np.array(base_apply, dtype=float, copy=True)
    sample_pred = np.array(base_sample, dtype=float, copy=True)
    fit_mask = (fit_pos >= lo) & (fit_pos < hi)
    apply_mask = (apply_pos >= lo) & (apply_pos < hi)
    sample_mask = (sample_pos >= lo) & (sample_pos < hi)
    if int(fit_mask.sum()) < min_rows or len(np.unique(y_fit[fit_mask])) < 2:
        return np.clip(apply_pred, EPS, 1.0 - EPS), np.clip(sample_pred, EPS, 1.0 - EPS)
    model = make_view_context_model(method)
    model.fit(fit_features[fit_mask], y_fit[fit_mask])
    if apply_mask.any():
...
def train_joint_encoder(args: argparse.Namespace) -> None:
    ...
    source_names = [
        ...
        "joint_residual_behavior_neural_knn_logitresid",
        "joint_residual_behavior_neural_metric_knn_resid",
        "joint_residual_behavior_neural_metric_knn_logitresid",
        "joint_residual_behavior_neural_multiview_knn_resid",
        "joint_residual_behavior_neural_multiview_knn_logitresid",
        "joint_residual_behavior_neural_multiview_metric_knn_resid",
        "joint_residual_behavior_neural_multiview_metric_knn_logitresid",
        "joint_family_residual_behavior_neural_knn_resid",
        "joint_family_residual_behavior_neural_knn_logitresid",
        "joint_family_residual_behavior_neural_metric_knn_resid",
        "joint_family_residual_behavior_neural_metric_knn_logitresid",
        "joint_cross_family_residual_behavior_neural_knn_resid",
        "joint_cross_family_residual_behavior_neural_knn_logitresid",
        "joint_cross_family_residual_behavior_neural_metric_knn_resid",
        "joint_cross_family_residual_behavior_neural_metric_knn_logitresid",
        "joint_target_residual_behavior_neural_knn_resid",
        "joint_target_residual_behavior_neural_knn_logitresid",
        "joint_target_residual_behavior_neural_metric_knn_resid",
        "joint_target_residual_behavior_neural_metric_knn_logitresid",
        "joint_s23_residual_behavior_neural_knn_resid",
        "joint_s23_residual_behavior_neural_knn_logitresid",
        "joint_s23_residual_behavior_neural_metric_knn_resid",
        "joint_s23_residual_behavior_neural_metric_knn_logitresid",
        "joint_s23_late_residual_behavior_neural_knn_resid",
        "joint_s23_late_residual_behavior_neural_knn_logitresid",
        "joint_s23_late_residual_behavior_neural_metric_knn_resid",
        "joint_s23_late_residual_behavior_neural_metric_knn_logitresid",
        "joint_target_late_residual_behavior_neural_knn_resid",
        ...
        "joint_neural_context_secondhalf_gate_logreg_resid",
    ...
    for fold_id, fold in enumerate(folds):
        ...
        residual_neighbor_targets, residual_neighbor_meta = residual_neighborhood_targets(
            residual_fit_targets,
            panel_residual_targets,
            k=max(17, args.knn_k + 12),
        )
        rbnrz_fit, rbnrz_val, rbnrz_sample, residual_behavior_neural_meta = fit_neural_residual_latent(
            fit_x,
            residual_neighbor_targets,
            val_x,
            sample_x,
            args.max_features,
            args.neural_latent_dim,
        ...
        residual_neighbor_mv_targets = np.hstack([residual_neighbor_targets, residual_proto_targets, rz_fit, qrz_fit, srz_fit, qsrz_fit])
        rbmnrz_fit, rbmnrz_val, rbmnrz_sample, residual_behavior_multiview_neural_meta = fit_neural_residual_latent(
            fit_x,
            residual_neighbor_mv_targets,
            val_x,
            sample_x,
            args.max_features,
            args.neural_latent_dim,
        ...
        q_panel_residual_targets = np.hstack(
            [residual_fit_targets[:, :3] * fit_panel_basis[:, [basis_i]] for basis_i in range(fit_panel_basis.shape[1])]
        )
        s_panel_residual_targets = np.hstack(
            [residual_fit_targets[:, 3:] * fit_panel_basis[:, [basis_i]] for basis_i in range(fit_panel_basis.shape[1])]
        )
        q_residual_neighbor_targets, q_residual_neighbor_meta = residual_neighborhood_targets(
            residual_fit_targets[:, :3],
            q_panel_residual_targets,
            k=max(17, args.knn_k + 12),
        )
        s_residual_neighbor_targets, s_residual_neighbor_meta = residual_neighborhood_targets(
            residual_fit_targets[:, 3:],
            s_panel_residual_targets,
            k=max(17, args.knn_k + 12),
        )
        qrbnrz_fit, qrbnrz_val, qrbnrz_sample, q_residual_behavior_neural_meta = fit_neural_residual_latent(
            fit_x,
            np.hstack([q_residual_neighbor_targets, qrz_fit, qsrz_fit]),
            val_x,
            sample_x,
            args.max_features,
            max(3, min(args.neural_latent_dim, 8)),
        ...
        srbnrz_fit, srbnrz_val, srbnrz_sample, s_residual_behavior_neural_meta = fit_neural_residual_latent(
            fit_x,
            np.hstack([s_residual_neighbor_targets, srz_fit, qsrz_fit]),
            val_x,
            sample_x,
            args.max_features,
            max(4, min(args.neural_latent_dim, 8)),
        ...
        s23_indices = [TARGET_COLUMNS.index("S2"), TARGET_COLUMNS.index("S3")]
        s23_residual_targets = residual_fit_targets[:, s23_indices]
        s23_panel_residual_targets = np.hstack(
            [s23_residual_targets * fit_panel_basis[:, [basis_i]] for basis_i in range(fit_panel_basis.shape[1])]
        )
        ...
        s23_late_panel_targets = np.hstack(
            [
                ...
                s23_residual_targets * np.square(late_basis),
                s23_residual_targets * mid_basis,
            ]
        )
        s23_residual_neighbor_targets, s23_residual_neighbor_meta = residual_neighborhood_targets(
            s23_residual_targets,
            s23_panel_residual_targets,
            k=max(23, args.knn_k + 18),
        )
        s23_late_residual_neighbor_targets, s23_late_residual_neighbor_meta = residual_neighborhood_targets(
            s23_residual_targets * (1.0 + 2.0 * late_basis),
        ...
        s23lbnrz_fit, s23lbnrz_val, s23lbnrz_sample, s23_late_residual_behavior_neural_meta = fit_neural_residual_latent(
            fit_x,
            np.hstack([s23_late_residual_neighbor_targets, s23_residual_neighbor_targets, srz_fit, qsrz_fit]),
            val_x,
            sample_x,
            args.max_features,
        ...
        meta = {
            ...
            **residual_neighbor_meta,
            "residual_behavior_neural_latent_dim": residual_behavior_neural_meta["neural_latent_dim"],
            "residual_behavior_neural_best_loss": residual_behavior_neural_meta["neural_best_loss"],
            "residual_behavior_multiview_neural_latent_dim": residual_behavior_multiview_neural_meta["neural_latent_dim"],
            "residual_behavior_multiview_neural_best_loss": residual_behavior_multiview_neural_meta["neural_best_loss"],
            "q_residual_neighborhood_target_dim": q_residual_neighbor_meta["residual_neighborhood_target_dim"],
            "q_residual_neighborhood_mean_distance": q_residual_neighbor_meta["residual_neighborhood_mean_distance"],
            "s_residual_neighborhood_target_dim": s_residual_neighbor_meta["residual_neighborhood_target_dim"],
            "s_residual_neighborhood_mean_distance": s_residual_neighbor_meta["residual_neighborhood_mean_distance"],
            "q_residual_behavior_neural_latent_dim": q_residual_behavior_neural_meta["neural_latent_dim"],
            "q_residual_behavior_neural_best_loss": q_residual_behavior_neural_meta["neural_best_loss"],
            "s_residual_behavior_neural_latent_dim": s_residual_behavior_neural_meta["neural_latent_dim"],
            "s_residual_behavior_neural_best_loss": s_residual_behavior_neural_meta["neural_best_loss"],
            "s23_residual_neighborhood_target_dim": s23_residual_neighbor_meta["residual_neighborhood_target_dim"],
            "s23_residual_neighborhood_mean_distance": s23_residual_neighbor_meta["residual_neighborhood_mean_distance"],
            "s23_late_residual_neighborhood_target_dim": s23_late_residual_neighbor_meta["residual_neighborhood_target_dim"],
            "s23_late_residual_neighborhood_mean_distance": s23_late_residual_neighbor_meta["residual_neighborhood_mean_distance"],
            "s23_residual_behavior_neural_latent_dim": s23_residual_behavior_neural_meta["neural_latent_dim"],
        ...
        for target_i, target in enumerate(TARGET_COLUMNS):
            ...
            target_panel_residual = np.hstack([target_residual * fit_panel_basis[:, [basis_i]] for basis_i in range(fit_panel_basis.shape[1])])
            target_residual_neighbor_targets, _ = residual_neighborhood_targets(
                target_residual,
                target_panel_residual,
                k=max(17, args.knn_k + 12),
            )
            tbrnrz_fit, tbrnrz_val, tbrnrz_sample, _ = fit_neural_residual_latent(
                fit_x,
                np.hstack([target_residual_neighbor_targets, target_residual, trz_fit, family_pls_fit, qsrz_fit]),
                val_x,
                sample_x,
                args.max_features,
                max(2, min(args.neural_latent_dim, 5)),
            ...
            target_late_panel_residual = np.hstack(
                [
                    target_residual * late_basis,
                    target_residual * np.square(late_basis),
                    target_residual * mid_basis,
                ]
            )
            target_late_neighbor_targets, _ = residual_neighborhood_targets(
                target_residual * (1.0 + 2.0 * late_basis),
                target_late_panel_residual,
                k=max(17, args.knn_k + 12),
            )
            tlbrnrz_fit, tlbrnrz_val, tlbrnrz_sample, _ = fit_neural_residual_latent(
                fit_x,
                np.hstack([target_late_neighbor_targets, target_residual_neighbor_targets, target_residual, trz_fit, family_pls_fit]),
                val_x,
                sample_x,
                args.max_features,
                max(2, min(args.neural_latent_dim, 5)),
            ...
            oof_by_source["joint_residual_behavior_neural_knn_resid"][fold.val_idx, target_i] = weighted_knn_residual(
                rbnrz_fit, rbnrz_val, y_fit, base_fit, base_val, args.knn_k, args.knn_temp, False
            )
            sample_folds_by_source["joint_residual_behavior_neural_knn_resid"].append(
                (target_i, weighted_knn_residual(rbnrz_fit, rbnrz_sample, y_fit, base_fit, base_test, args.knn_k, args.knn_temp, False))
            )
            oof_by_source["joint_residual_behavior_neural_knn_logitresid"][fold.val_idx, target_i] = weighted_knn_residual(
                rbnrz_fit, rbnrz_val, y_fit, base_fit, base_val, args.knn_k, args.knn_temp, True
            )
            sample_folds_by_source["joint_residual_behavior_neural_knn_logitresid"].append(
                (target_i, weighted_knn_residual(rbnrz_fit, rbnrz_sample, y_fit, base_fit, base_test, args.knn_k, args.knn_temp, True))
            )
            oof_by_source["joint_residual_behavior_neural_multiview_knn_resid"][fold.val_idx, target_i] = weighted_knn_residual(
                rbmnrz_fit, rbmnrz_val, y_fit, base_fit, base_val, args.knn_k, args.knn_temp, False
            )
            sample_folds_by_source["joint_residual_behavior_neural_multiview_knn_resid"].append(
                (target_i, weighted_knn_residual(rbmnrz_fit, rbmnrz_sample, y_fit, base_fit, base_test, args.knn_k, args.knn_temp, False))
            )
            oof_by_source["joint_residual_behavior_neural_multiview_knn_logitresid"][fold.val_idx, target_i] = weighted_knn_residual(
                rbmnrz_fit, rbmnrz_val, y_fit, base_fit, base_val, args.knn_k, args.knn_temp, True
            )
            sample_folds_by_source["joint_residual_behavior_neural_multiview_knn_logitresid"].append(
                (target_i, weighted_knn_residual(rbmnrz_fit, rbmnrz_sample, y_fit, base_fit, base_test, args.knn_k, args.knn_temp, True))
            )
            residual_behavior_weights = residual_metric_weights(rbnrz_fit, y_fit - base_fit)
            residual_behavior_mv_weights = residual_metric_weights(rbmnrz_fit, y_fit - base_fit)
            oof_by_source["joint_residual_behavior_neural_metric_knn_resid"][fold.val_idx, target_i] = metric_weighted_knn_residual(
                rbnrz_fit, rbnrz_val, y_fit, base_fit, base_val, residual_behavior_weights, args.knn_k, args.knn_temp, False
            )
            sample_folds_by_source["joint_residual_behavior_neural_metric_knn_resid"].append(
                (
                    target_i,
                    metric_weighted_knn_residual(
                        rbnrz_fit, rbnrz_sample, y_fit, base_fit, base_test, residual_behavior_weights, args.knn_k, args.knn_temp, False
                    ),
                )
            )
            oof_by_source["joint_residual_behavior_neural_metric_knn_logitresid"][fold.val_idx, target_i] = metric_weighted_knn_residual(
                rbnrz_fit, rbnrz_val, y_fit, base_fit, base_val, residual_behavior_weights, args.knn_k, args.knn_temp, True
            )
            sample_folds_by_source["joint_residual_behavior_neural_metric_knn_logitresid"].append(
                (
                    target_i,
                    metric_weighted_knn_residual(
                        rbnrz_fit, rbnrz_sample, y_fit, base_fit, base_test, residual_behavior_weights, args.knn_k, args.knn_temp, True
                    ),
                )
            )
            oof_by_source["joint_residual_behavior_neural_multiview_metric_knn_resid"][fold.val_idx, target_i] = metric_weighted_knn_residual(
                rbmnrz_fit, rbmnrz_val, y_fit, base_fit, base_val, residual_behavior_mv_weights, args.knn_k, args.knn_temp, False
            )
            sample_folds_by_source["joint_residual_behavior_neural_multiview_metric_knn_resid"].append(
                (
                    target_i,
                    metric_weighted_knn_residual(
                        rbmnrz_fit, rbmnrz_sample, y_fit, base_fit, base_test, residual_behavior_mv_weights, args.knn_k, args.knn_temp, False
                    ),
                )
            )
            oof_by_source["joint_residual_behavior_neural_multiview_metric_knn_logitresid"][fold.val_idx, target_i] = metric_weighted_knn_residual(
                rbmnrz_fit, rbmnrz_val, y_fit, base_fit, base_val, residual_behavior_mv_weights, args.knn_k, args.knn_temp, True
            )
            sample_folds_by_source["joint_residual_behavior_neural_multiview_metric_knn_logitresid"].append(
                (
                    target_i,
                    metric_weighted_knn_residual(
                        rbmnrz_fit, rbmnrz_sample, y_fit, base_fit, base_test, residual_behavior_mv_weights, args.knn_k, args.knn_temp, True
                    ),
                )
            )
            family_behavior_fit = qrbnrz_fit if target_i < 3 else srbnrz_fit
            family_behavior_val = qrbnrz_val if target_i < 3 else srbnrz_val
            family_behavior_sample = qrbnrz_sample if target_i < 3 else srbnrz_sample
            ...
            for source_prefix, behavior_fit, behavior_val, behavior_sample in (
                ...
                ("joint_target_residual_behavior_neural", tbrnrz_fit, tbrnrz_val, tbrnrz_sample),
                ...
                sample_folds_by_source[f"{source_prefix}_knn_resid"].append(
                    (target_i, weighted_knn_residual(behavior_fit, behavior_sample, y_fit, base_fit, base_test, args.knn_k, args.knn_temp, False))
                )
                oof_by_source[f"{source_prefix}_knn_logitresid"][fold.val_idx, target_i] = weighted_knn_residual(
                    behavior_fit, behavior_val, y_fit, base_fit, base_val, args.knn_k, args.knn_temp, True
                )
                sample_folds_by_source[f"{source_prefix}_knn_logitresid"].append(
                    (target_i, weighted_knn_residual(behavior_fit, behavior_sample, y_fit, base_fit, base_test, args.knn_k, args.knn_temp, True))
                )
                behavior_weights = residual_metric_weights(behavior_fit, y_fit - base_fit)
                oof_by_source[f"{source_prefix}_metric_knn_resid"][fold.val_idx, target_i] = metric_weighted_knn_residual(
                    behavior_fit, behavior_val, y_fit, base_fit, base_val, behavior_weights, args.knn_k, args.knn_temp, False
                )
                sample_folds_by_source[f"{source_prefix}_metric_knn_resid"].append(
                    (
                        target_i,
                        metric_weighted_knn_residual(
                            behavior_fit, behavior_sample, y_fit, base_fit, base_test, behavior_weights, args.knn_k, args.knn_temp, False
                        ),
                    )
                )
                oof_by_source[f"{source_prefix}_metric_knn_logitresid"][fold.val_idx, target_i] = metric_weighted_knn_residual(
                    behavior_fit, behavior_val, y_fit, base_fit, base_val, behavior_weights, args.knn_k, args.knn_temp, True
                )
                sample_folds_by_source[f"{source_prefix}_metric_knn_logitresid"].append(
                    (
                        target_i,
                        metric_weighted_knn_residual(
                            behavior_fit, behavior_sample, y_fit, base_fit, base_test, behavior_weights, args.knn_k, args.knn_temp, True
                        ),
                    )
                )
            ...
            mixture_fit, mixture_val, mixture_sample = stack_and_scale_latents(
                ...
                [
                    ...
                    rbnrz_sample,
                    rbmnrz_sample,
                    family_behavior_sample,
                    cross_family_behavior_sample,
                    tbrnrz_sample,
                ],
            )
            ...
            neural_views = [
                ...
                ("residual_behavior", rbnrz_fit, rbnrz_val, rbnrz_sample),
                ("residual_behavior_multiview", rbmnrz_fit, rbmnrz_val, rbmnrz_sample),
                ("family_residual_behavior", family_behavior_fit, family_behavior_val, family_behavior_sample),
                ("cross_family_residual_behavior", cross_family_behavior_fit, cross_family_behavior_val, cross_family_behavior_sample),
                ("target_residual_behavior", tbrnrz_fit, tbrnrz_val, tbrnrz_sample),
            ...
            for method in ("logreg", "hgb"):
                ...
                val_pred, sample_pred = fit_view_context_window_decoder(
                    method,
                    view_fit_preds_resid,
                    y_fit,
                    base_fit,
                ...
                source_name = f"joint_neural_context_secondhalf_gate_{method}_resid"
                oof_by_source[source_name][fold.val_idx, target_i] = val_pred
                sample_folds_by_source[source_name].append((target_i, sample_pred))
