...
This keeps producing new OOF signal after earlier layers are already strong. The latest neural residual-view experiments suggest the useful latent is not one flat vector yet; it is a small set of specialized target/panel/prototype views that repeatedly recover new residual layers.
...
| conditional routing v80 family/cross-family late residual-neighborhood objective | 0.477600 |
...
- A family/cross-family late residual-neighborhood objective improves the v79 base from 0.478230 to 0.477600 in all-source routing. Neural-only reaches 0.477768, behavior-only reaches 0.477855, and new-late-only reaches 0.477997. The new late-only signal selects Q1 mid from Q-family late behavior and S2 mid/S3 second-half from cross-family late behavior, so the late residual-behavior manifold generalizes beyond target-local S23.

## Best Current Breakthrough Candidate

- Bold/best OOF submission: `outputs/conditional_latent_routing_v80_late_behavior_on_v79/submission_conditional_latent_routing.csv`
- Bold/best OOF: `0.477600`
- Bold/best report: `outputs/conditional_latent_routing_v80_late_behavior_on_v79/report.md`
- Cleaner/neural-only OOF candidate: `outputs/conditional_latent_routing_v80_neural_only_on_v79/submission_conditional_latent_routing.csv`
- Cleaner/neural-only OOF: `0.477768`
- Residual-behavior-only evidence candidate: `outputs/conditional_latent_routing_v80_behavior_only_on_v79/submission_conditional_latent_routing.csv`
- Residual-behavior-only OOF: `0.477855`
- New-late-only evidence candidate: `outputs/conditional_latent_routing_v80_new_late_only_on_v79/submission_conditional_latent_routing.csv`
- New-late-only OOF: `0.477997`
...
- family/cross-family late residual-neighborhood objective: confirms the late-behavior manifold transfers across label families, especially Q-family late to Q1 and Q-late/cross-family late into S2/S3
...
Family/cross-family late residual-neighborhood update: newest best score is 0.477600. The new-late-only route reaches 0.477997, selecting Q1 mid from family-late behavior and S2 mid/S3 second-half from cross-family-late behavior. This confirms the late residual-behavior manifold is a real encoder direction, not only a target-local S23 artifact; the next experiment should stress-test it under stricter sample-position gates and preserve cross-family transfer.
...
- v81 decoder update: the newest best score is 0.471378, a -0.006222 jump from the v80 base and ~4x any prior single-version gain. This is the first *real* decoder layer rather than a manual router over raw KNN sources: target-wise residual decoders (Ridge/HGB/ExtraTrees/logreg) read fold-safe latent retrieval summaries (a global pool plus a late-panel pool), early/mid/late panel basis, the v80 base, and a curated set of v80 source predictions, and each predicts the residual against the v80 base. Decoder-only routing improves every target: Q1 (-0.019076), S3 (-0.006066), Q2 (-0.004824), S2 (-0.003622), S4 (-0.003778), Q3 (-0.003621), and S1 (-0.002560). The gain is carried almost entirely by `v81_hgb`, the nonlinear HistGradientBoosting residual decoder; the linear/logreg/extratrees variants are weak or neutral, exactly as predicted, because the router already approximates linear target/bin blending and the only new signal is the nonlinear retrieval x panel x base interaction. All-source routing (v81 decoders plus the v80 source pool) equals decoder-only and selects only the v81 decoder, so the learned decoder absorbs the entire v80 raw-KNN source pool. Leakage was verified clean end to end: matched fold partition with the v80 encoder, key-based self-exclusion in retrieval, fold-safe decoder OOF, fold-safe v80 source-pred features, and a fold-safe OOF latent (PCA/PLS fit on fit pool only, written only for val rows). The current best thesis is now: the final composer should be a single nonlinear residual decoder over fold-safe latent-retrieval features with its own panel/target gating, not a hand router over many raw KNN sources — but the 0.006 jump is internal OOF only and must be packaged conservatively and checked against the v76 Public-LB anchor before it is trusted.
- v81 stress-test correction: the earlier "leakage verified clean end to end" claim was too strong. The decoder OOF is fold-safe, but the routed 0.471378 is produced by the conditional router selecting (source, bin, weight) on full-train OOF — that selection sees labels and is optimistic. Correct framing: **fold-safe decoder, but prior OOF router selection bias remains.** Measured (`outputs/v81_selection_bias_stress_report.md`): router selection bias is +0.0034 (train-selected in-sample 0.471070 vs nested out-of-fold-selected 0.474428); selection-free uniform fixed shrinkage reaches 0.472080 (s=0.25). Nested-validated honest per-target gains are Q1 (-0.0191) and S3 (-0.0058) plus a tiny S2; the Q3/S1/S4 routed gains did NOT survive out-of-fold selection and were selection bias. Q1 attribution: the gain is v80 source-prediction recombination, not neighbor-label smoothing — retrieval_only gives Q1 +0.0003 (no signal) while no_retrieval (source-preds + panel + base) gives Q1 +0.0316, exceeding the full decoder. The retrieval features (especially the late-pool) are net harmful: fixed-shrinkage avg gain is full -0.0055 < no_retrieval -0.0079 < no_late -0.0099. The corrected thesis: the next decoder should be retrieval-free (at most geometry), recombine the v80 source predictions nonlinearly, and always be evaluated by nested/out-of-fold selection rather than full-train router selection.
