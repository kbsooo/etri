...
- Observe: every known public LB is not just a score. Relative to E247, each submission creates a log-loss equation over hidden public labels and public subset weighting.
- Wonder: can old public observations be inverted into a pseudo-public label/subset representation strong enough to produce a non-micro submission?
- Hypothesis: if the known public observations are generated by a stable hidden public state, then solving the public-equation system should recover a useful target representation. Moving E247 toward that representation should create a large predicted improvement under leave-one-public stress. If public feedback fails, the old public equations are too underidentified or subset-mismatched for direct materialization.
- Method: `hitl/h012_public_equation_jepa_jackpot.py`.
  - base: E247;
  - equations: `19` known-public deltas versus E247;
  - latent target: continuous hidden public label posterior `q`;
  - priors: E247, good-public soft ensemble, good-public median, neutral, sharp E247;
  - posterior selection: ridge dual solve with leave-one-public-file-out diagnostics;
  - action families: direct all-cell posterior move, all/Q/S top-cell moves, target-specific top-cell moves, consistency-gated moves.
- Result:
  - known public observations loaded: `20`;
  - posterior configs tested: `85`;
  - top config LOO MAE `0.000320737`, LOO p90 abs `0.000911893`, Spearman `0.935088`;
  - generated candidates: `238`;
  - selected upload-safe file: `submission_h012_public_equation_top_all_k1200_a0.7_uploadsafe.csv`;
  - selected action changes `1200` cells across all targets;
  - posterior mean/p90 delta vs E247: `-0.006446397` / `-0.004693170`;
...
- Observe: H012's public win proves that known LB observations contain a hidden public-state signal, but it does not say whether the signal is mainly hidden labels or hidden public cell weighting.
- Wonder: can the known public scores also identify which row x target cells the public sensor listens to?
- Hypothesis: if public LB is generated by a stable hidden public cell-weight field, then fitting weights from known submission loss-deltas should predict held-out public deltas and survive a permutation/null stress where the public scores are detached from the submission tensors.
- Method: `hitl/h016_public_subset_weight_jepa.py`.
  - current anchor: `submission_h012_public_equation_top_all_k1200_a0.7_uploadsafe.csv`;
  - candidate direction: H015 self-feedback movement;
  - target: public cell weights over `250 x 7 = 1750` test cells;
  - label proxies: H012/H015 probabilities and H012/H015 public-equation posteriors;
  - action: apply H015 movement only on cells with high inferred public-weight/gain score.
- Result:
  - selected weight config: `h012_median_posterior`, ridge `0.001`, cap `12`;
  - LOO MAE `0.000013654`, p90 abs `0.000026381`, Spearman `0.990977444`;
  - uniform-weight MAE `0.000885430`;
  - effective public weight count `1747.348299`, top50 weight mass `0.036149`, so the signal is diffuse rather than a tiny hard public subset;
  - permutation null stress: null median LOO MAE `0.004329919`, null p90 `0.008450378`, max null Spearman `0.660150`; real beats all `300` permutations on MAE and Spearman;
  - primary upload-safe file: `submission_h016_public_subset_gain_all_k1000_a0.75_uploadsafe.csv`;
  - primary changes `1000` cells across all targets with max probability delta `0.040887`;
  - predicted subset-weight delta vs H012 `-0.000296297`, while full H015 is predicted `+0.000164649` under this same weight model.
- Interpretation:
  - H016 is not just an underdetermined equation artifact under the tested null; the real public score ordering is tightly coupled to the submission loss-delta tensor.
  - It contradicts H015's broad sharpening worldview: if H016 is right, the right action is not "sharpen H012 everywhere", but "use H015 only on public-weight/gain-compatible cells."
  - The upside is smaller than H015, but the world-model value is high because it separates hidden label posterior from hidden public-cell weighting.
- Decision:
  - H016 is a valid high-information alternative sensor to H015.
  - H015 remains the higher-upside recursive sharpening bet.
  - H016 should be preferred if the next question is "is public feedback a diffuse cell-weight field rather than a new label posterior?"

## H017. Joint Label x Public-Weight HS-JEPA

- Observe: H012's label posterior and H016's weight field each explain public feedback from different projections. The unresolved question is whether they are compatible parts of one hidden public world.
- Wonder: if public LB is generated by `sum_cell w[cell] * loss_delta(pred, q[cell])`, do H012's public posterior and H016's diffuse weights form a stable joint latent, and does that imply H012 did not move far enough toward its own posterior?
- Hypothesis: a joint `(q, w)` representation should predict held-out public deltas better than either uniform weighting or permuted public-score nulls. If the solution barely moves from the H012/H016 priors, that means the priors are mutually compatible rather than independently lucky.
- Method: `hitl/h017_joint_label_weight_jepa.py`.
  - current anchor: H012;
  - latent variables: public label posterior `q` and public cell weights `w`;
  - priors: H012/H015/H016 probabilities, H012/H015 public posteriors, top-public soft priors, H016 mean/median/rank weights;
  - solver: ridge-dual joint fit over `[w, w*q]`, projected to valid probability and high-entropy weight constraints;
  - null: permute known public deltas `300` times while keeping submission loss-delta tensors fixed;
  - action: move H012 further toward joint `q` on cells with high joint public-weighted gain.
- Result:
  - selected config: `q=h012_public_posterior`, `w=h016_mean_weight`, ridge `0.1`, cap `8`;
  - LOO MAE `0.000001044`, p90 abs `0.000002071`, Spearman `1.000000`;
  - permutation null median LOO MAE `0.001672425`, max null Spearman `0.200902`;
  - real beats all `300` permutations on MAE and Spearman;
  - critical caveat: `q_prior_abs_move=0.000000677` and `w_prior_l1_move=0.000000293`, so H017 did not discover a new joint state. It proved H012 posterior plus H016 weights are already mutually compatible;
  - primary file: `submission_h017_joint_label_weight_oracle_gain_all_k1650_a1_uploadsafe.csv`;
  - primary changes `1650` cells, max probability delta `0.107121`, mean absolute delta `0.011129`;
  - joint-world predicted delta vs H012 `-0.000574501`;
  - under the same joint sensor, H015 is `+0.000164654` and H016 is `-0.000296289`.
- Interpretation:
  - H017 is best read as posterior-completion, not a brand-new latent. H012 may have been an under-moved step toward the H012 public posterior, and H016 identifies the diffuse weight field under which continuing that movement helps.
  - It is more aggressive than H016 and less self-feedback-driven than H015.
  - The public result would separate three worldviews: H015 recursive sharpening, H016 weighted H015 slice, and H017 H012-posterior completion.
- Decision:
