...
E48-E81 이후에는 기준점이 다시 좁아졌다. Mixmin은 previous best를 `0.0011326805` 이겼고, E49-E79는 mixmin 이후의 block-state, raw context, mixmin-hard world, Q2/S3 tail-risk, sparse consensus, amplitude, posterior averaging, reliability mask, row/block/flank mask 가설을 차례로 좁혔다. E80의 public observation은 `submission_e72_topabs50_q2s3_gate_4e48cba2.csv`가 `0.5764077772`로 mixmin보다 `+0.0001011367` 나빴음을 기록했다. 더 중요한 점은 이 파일이 순수 Q2/S3가 아니라 `893` cells / `249` rows / all `7` targets를 움직였다는 것이다. E81은 이를 분리했고, pure Q2/S3 graft는 loose지만 sub-margin(`-5.95383e-6`), inverse Q2/S3는 local stress에서 명확히 adverse(`+1.47473e-5`)였다. 그러므로 다음 질문은 더 이상 "E74/E75로 바로 amplitude를 키울까"가 아니다. 남은 질문은 "public이 거부한 broad all-target base movement를 제거하면서, sub-margin Q2/S3 latent를 combo-set/tail-calibrated structural gate로 margin-scale까지 키울 수 있는가"다.
