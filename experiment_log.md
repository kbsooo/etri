...
- Decision: keep E73 as the first public sensor if only one slot is used. Treat `submission_e74_fullpool_a20_q2s3_gate_55455b60.csv` as a secondary higher-upside/higher-risk amplitude sensor: if E73 improves but E74 worsens, sparse gate is real but alpha20 oversteps; if E74 improves more, E73 was under-amplified.
...
- Decision: add H74/F60/FH69. No E78 submission. Keep E73 as first public sign sensor, E75 as target-asymmetry sensor, and E74 as symmetric control. Next amplitude localization needs richer public-like row/block/tail structure or public feedback.
...
- Decision: add H75/F61/FH70. No E79 submission. Public test order is unchanged: E73 is the first sparse-gate sign sensor; E75 is the target-asymmetric amplitude sensor; E74 is the symmetric control. Future localization must use combo-set/tail calibration or a learned structural target, not handcrafted row/block/flank masks alone.
...
E48-E79 이후에는 기준점이 바뀌었다. Mixmin은 previous best를 `0.0011326805` 이겼고, E49는 그 움직임이 단순 global/subject/recent prior correction으로 설명되지 않으며 subject-calendar hidden test block과 더 잘 맞는다는 관찰을 남겼다. E50은 그 관찰을 바로 selector로 쓰는 시도를 반증했다: best subject-calendar selector도 mixmin을 public-best로 예측하지 못했다. E51은 anchor-loss/binary-world geometry를 결합해도 같은 selector 변환이 실패함을 보였다. E52는 방향을 바꿔 mixmin을 관측 constraint로 놓고 기존 후보들의 sign을 테스트했지만, 158개 후보 중 strict/loose better-than-mixmin은 `0`이었다. E53은 남은 calendar-flank block-rate/count branch의 가장 단순한 count-state posterior를 찔렀다. Local/same-subject donor는 pseudo-hidden에서 `-0.005266` 개선됐지만 strict donor는 `+0.001434` 나빠졌고, 실제 hidden alignment도 strict 기준 `-0.000179`로 약했다. E54는 raw overnight context를 추가하면 strict pseudo-hidden block-state recovery가 강하게 살아난다는 점을 보였다(`-0.007733`). 그러나 그 latent는 mixmin-public latent가 아니었다. Best raw strict method는 실제 hidden mixmin alignment를 `+0.000311`로 악화시켰고 S3도 pseudo-hidden에서 adverse였다. E55는 이 conflict가 단순 Q/S target-dependency projection으로 고쳐지는지 찔렀고, joint gate `0/225`로 반증했다. E56은 mixmin을 hard constraint로 넣은 새 binary-world family를 실제로 만들었고, world-LOO posterior gate `12`개를 열었다. 하지만 E57은 그 posterior를 직접 제출하는 가설을 반증했다: joint safety gate는 `0`, 가장 안전한 posterior도 actual-anchor에서 mixmin보다 `+0.000123` 나빴고, E56 selected candidate는 `+0.020381`로 크게 나빴다. E58은 단순 distillation을 시도했지만 submission margin gate는 `0`이었고, E61은 그 결론이 scoring-index artifact가 아님을 확인했다. E59는 within-block joint label-pattern representation을 찔렀고, joint 구조 자체는 강하게 살아났지만 row calibration과 hidden mixmin sign으로 번역되지 않았다. E60은 transition residual이 hidden sign 센서로는 더 강하지만 row calibration을 완전히 깨뜨린다는 점을 보였다. E62는 transition residual을 target이 아니라 E56 gate로만 써도 selector-scale margin이 생기지 않는다는 점을 보였다. E63은 독립 hidden-rate gradient consensus가 E56 방향 자체를 강하게 지지하지만, 그 방향도 아직 public-anchor margin으로 번역되지 않는다는 점을 보였다. E64는 그 방향을 더 크게 키우면 된다는 단순 amplitude 가설을 반증했다. E65는 near-zero targetwise pocket이 존재하지만 여전히 sub-margin임을 보였다. E66은 그 pocket이 Q2/S3 hidden-direction 오류 때문이라는 단순 해석을 반증했다. E67은 first-order anchor-tail gate가 matched `no_q2_s3`보다 낫다는 점을 보였지만, 최고 `-6.933e-6`로 여전히 margin을 넘지 못했다. E68은 그 tail-gated Q2/S3 cell이 known-anchor derivative 밖에서도 살아남는지 찔렀고, held-out combo construction plus hidden/world/block stress에서 independent/strict gate `155/540`을 확인했다. E69는 그 strict cell을 단순 alpha로 키우는 가설을 반증했다. E70은 strict cell consensus가 margin을 살짝 넘는 local structure를 만들 수 있음을 보였지만, `gate=none`과 heldout-specific construction 때문에 바로 제출할 수는 없었다. E71은 unique-cell full-combo reconstruction으로 이 objection을 줄였지만 deployable unified gate는 `0`이었다. E72가 이 결론을 다시 갈랐다: sign-agreement gate는 여전히 부족하지만 `top_abs50` sparse-magnitude gate는 `7`개, `s3_only`는 `3`개의 deployable rows를 만들었다. E73은 그중 best deployable row를 `submission_e72_topabs50_q2s3_gate_4e48cba2.csv`로 물질화했다. E74는 이 sparse gate가 단일-cell artifact일 가능성을 크게 줄였다: jackknife alpha16은 `13/13` deployable, bootstrap8 alpha16은 `48/60` deployable이었다. E75는 amplitude 병목을 한 번 더 갈랐다. Symmetric alpha20이 국소 최적이 아니었고, best deployable은 Q2를 `8`로 줄이고 S3를 `28`로 키운 `submission_e75_q2a8_s3a28_sparse_amp_f07219b4.csv`였다. E76은 이 해석을 정밀화했다: S3-heavy direction은 모든 `94/94` subset variants에서 symmetric controls를 이겼지만, exact `8/28` deployability는 `49/94`라 E73 alpha16만큼 넓게 안정적이지는 않았다. E77은 그 불안정성을 source-subset posterior 평균으로 해결하는 길을 반증했다. Q2/S3-only posterior는 안전하지만 sub-margin이고, full posterior는 margin은 만들지만 combo-set/tail consistency를 깬다. E78은 source-subset reliability mask로 amplitude를 국소화하는 다음 길도 반증했다. `4452` rows 중 strict/deployable `1806`은 열렸지만 E75 local all-combo를 이기는 deployable row는 `0`이었다. E79는 남은 row/block/flank-localization 가설도 약화했다. `61`개 topology/energy/subject/flank mask와 `6516` rows에서 strict/deployable `1318`은 열렸지만 E75를 이기는 deployable row는 `0`이었고, E75 movement는 이미 `72/250` active sparse rows/cells 위에서 full-active local optimum으로 남았다. 그러므로 다음 질문은 이제 "어떤 non-`none` gate도 안 되나", "E76 posterior/신뢰도만 평균·마스킹하면 되나", "subject-calendar/flank mask만 자르면 되나"가 아니다. 남은 질문은 "sparse-magnitude Q2/S3 consensus가 public에서도 살아남는가, 살아남는다면 amplitude를 combo-set/tail calibration 또는 learned structural target으로 어떻게 조건화해야 하는가"다.
