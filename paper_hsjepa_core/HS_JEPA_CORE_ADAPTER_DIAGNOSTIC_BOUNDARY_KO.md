...
주의할 점은 6번, 7번, 8번이 pure core라기보다 core-decoder boundary라는 것이다. 여기서부터는 representation을 실제
row-target action으로 번역하는 문제가 들어오므로, 논문에서는 `HS-JEPA core가 action-health geometry를 제공하고,
tail-safe/subject-normalized/episode-conditioned decoder가 이를 Log Loss action으로 해석한다`고 써야 한다.
