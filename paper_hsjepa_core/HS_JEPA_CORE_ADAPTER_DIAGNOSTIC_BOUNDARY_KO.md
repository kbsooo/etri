...
- `Action-Free Vulnerability Gate Core`: action-aware decoder 없이 core context만으로 vulnerability를 gate할 수 있는지 검사한 negative core-boundary sensor
- `Counterfactual Directional Action-Health Core`: action probability/magnitude 없이 up/down direction listener만으로 hidden action-health를 예측할 수 있는지 검사한 negative core-boundary sensor
- `Core Student Recovers Masked Tail Teacher`: masked-view teacher hidden tail representation을 core student가 복원할 수 있는지 검사한 distillation-style negative sensor
- `Subject-Invariant Listener Responsibility Field Core`: action release 전에 row-target listener responsibility field를 복원하는 positive core-boundary evidence
- `Signed Listener Responsibility Direction Core`: responsibility field 위에서 raw/inverse signed action direction을 고르는 core-decoder boundary / adapter evidence
...
새 실험을 HS-JEPA라고 부르기 전에 아래 테스트를 통과해야 한다.

```text
1. context가 명시되어 있는가?
   보이는 lifelog / episode / listener / subject context가 무엇인지 써야 한다.

2. hidden target representation이 명시되어 있는가?
   raw label이나 submission probability가 아니라, masked view, tail field,
   action-health, listener responsibility 같은 중간 표현이어야 한다.

3. predictor가 context -> hidden representation 문제를 푸는가?
   단순 top-k correction, blend, threshold rule이면 core가 아니다.

4. energy 또는 health check가 있는가?
   predicted representation이 collapse/shortcut/toxic action인지 검사해야 한다.

5. competition adapter와 분리되어 있는가?
   Q1/Q2/Q3/S1/S2/S3/S4 release rule은 adapter로 내려가야 한다.
```

이 중 1-3을 통과하지 못하면 HS-JEPA core가 아니다.
4만 강하면 LeJEPA-style diagnostic이고, 5가 중심이면 competition adapter다.
...
- `ACTION_FREE_VULNERABILITY_GATE_CORE_KO.md`
- `COUNTERFACTUAL_DIRECTIONAL_ACTION_HEALTH_CORE_KO.md`
- `CORE_STUDENT_RECOVERS_MASKED_TAIL_TEACHER_KO.md`
...
10. action-free vulnerability gate
    action-free context -> row-target vulnerability representation -> action decoder gate

11. counterfactual directional action-health
    human-state context + up/down listener -> hidden directional action-health representation

12. core student recovers masked tail teacher
    human-state context + minimal action listener -> masked-view teacher hidden tail representation

13. subject-invariant listener responsibility field
    human-state context + target listener -> hidden row-target responsibility representation
```
...
`Action-Free Vulnerability Gate Core`는 중요한 반대 증거다. broad vulnerability는 action-free core context에서
어느 정도 읽히지만 nested subject-heldout gain은 `-3.006164`로 무너졌다. 따라서 HS-JEPA를
`core-only vulnerability predictor`라고 주장하면 안 된다. 현재 가장 엄밀한 주장은
`core context가 hidden state geometry를 만들고, masked-view/action-tail decoder boundary가 그 geometry를
release-safe action으로 번역한다`이다.

`Counterfactual Directional Action-Health Core`는 한 단계 더 구체적인 반대 증거다. action probability와
magnitude를 숨기고 up/down direction listener만 넣으면 full OOF는 `+3.026331`로 좋아 보이지만
nested subject-heldout은 `-3.515635`로 무너지고 stable target이 남지 않는다. 따라서 HS-JEPA를
`direction listener만으로 safe action을 고르는 core`라고 주장하면 안 된다. 현재 필요한 것은
direction보다 더 풍부한 action-tail representation 또는 masked-view action-tail consensus다.

`Core Student Recovers Masked Tail Teacher`는 masked-view teacher가 core student로 압축되는지 확인한
distillation-style 반대 증거다. full OOF는 `+8.078259`로 매우 강하지만 nested subject-heldout은
`-3.635973`으로 무너지고 stable target이 남지 않는다. 따라서 현재 frontier는
`core-only student가 teacher를 재현했다`가 아니라, `action-aware masked-view teacher boundary가 아직 필요하다`로
해석해야 한다.

`Subject-Invariant Listener Responsibility Field Core`는 hidden target을 action-level release에서
row-target listener responsibility로 낮춘 실험이다. masked-pretext responsibility AP lift는 `+0.079078`,
human responsibility AP lift는 `+0.077261`로 listener-only `+0.064292`를 넘었다. 이는 HS-JEPA core가
"어느 row-target에 listener가 개입해야 하는가"를 더 잘 복원한다는 강한 증거다. 하지만 기존 action decoder로
번역하면 OOF gain은 아직 `-0.565668`이므로, 이 실험은 core positive / action translation fragile로
읽어야 한다.

`Signed Listener Responsibility Direction Core`는 위 병목을 raw/inverse direction 선택 문제로 바꾼다.
결과적으로 responsibility-gated OOF gain이 `-0.565668`에서 `+1.640820`으로 바뀌었다. 하지만 best direction
family는 `action_geometry_direction`이므로, 이것은 pure core victory가 아니다. 정확한 해석은
`HS-JEPA responsibility core가 어디를 고를지 줄이고, signed action-direction adapter가 그 안에서 독성을 수리했다`이다.
...
- `HS-JEPA adapter`라고 불러야 하는 것:
  transport, route decoder, sparse correction, assignment solver, signed action-direction decoder, submission generator.
