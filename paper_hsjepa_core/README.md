...
- `paper_hsjepa_core/COUNTERFACTUAL_DIRECTION_PRETEXT_CORE_KO.md`
- `paper_hsjepa_core/HUMAN_STATE_WORLD_MODEL_CORE_KO.md`
- `paper_hsjepa_core/ROUTINE_BREAK_WORLD_MODEL_CORE_KO.md`
- `paper_hsjepa_core/SLEEP_PRESSURE_WORLD_MODEL_CORE_KO.md`
- `paper_hsjepa_core/COHORT_RELATIVE_WORLD_MODEL_CORE_KO.md`
...
`CROSS_SUBJECT_PROTOTYPE_TRANSPORT_CORE_KO.md`는 위 prototype grammar를 더 엄격하게 검증한 최신 positive 문서다. grammar를 full cohort에 한 번 맞춘 것이 아니라, fold마다 train subjects/blocks에서만 만든 뒤 held-out subject/block으로 운반한다.

현재 결론:

```text
train subjects가 정의한 subject-relative episode grammar는 held-out subject로 운반된다.
subject-heldout pretext mean cross-entropy lift는 +0.060052다.
transported stats+probabilities frozen probe는 prior 대비 -0.002026 logloss다.
stats-only probe도 prior 대비 -0.001411, raw lifelog PCA 대비 -0.002261이다.
stats-only subject-id accuracy는 0.273333으로 raw lifelog PCA 0.957778보다 낮다.
다만 probability-rich readout은 leakage가 커질 수 있어, transport 가능성과 readout risk를 분리해 써야 한다.
```
...
`SUBJECT_NORMALIZED_TAIL_FIELD_CORE_KO.md`는 tail-safe 실험의 negative sensor를 다음 target representation으로 바꾼다. absolute action gain을 바로 예측하지 않고, subject-target-action route 내부의 tail scale로 정규화한 `이 사람 기준의 relative badness`를 예측한다.

현재 핵심 결과:

- full OOF selected gain sum: `+2.898288`
- nested subject-heldout gain sum: `-3.812519`
- tail-safe expected utility의 nested heldout gain `-8.823949`보다 손실이 크게 줄었다.
...
```text
absolute utility를 맞히는 것보다 subject-normalized tail field를 맞히는 편이
subject shift에서 덜 독성적이다.
